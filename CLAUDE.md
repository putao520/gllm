...
| `SEMANTIC-GATEKEEPER.md` | **Semantic Gatekeeper 技术协议 (SSOT)** — Level Keys 预计算、Q-tap 截获、稳定性追踪、KnowledgeProvider trait、CallbackChain 集成、Mega-Kernel SgDetect/SgInject 共享内存、E2E 验收 (REQ-SG-001..008) | ✅ REQ-SG-001~007 已实现; ✅ REQ-MEGA-SG-001 已实现 (hook_ctx_ptr + SgSharedMemory); 🟡 REQ-SG-008 E2E 测试存在，行为差异验证待加强; 🟡 REQ-MEGA-SG-002 generate loop 拆分架构决策待定 |
