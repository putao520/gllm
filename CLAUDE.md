...
|---|----------|----------|---------|
| A1 | OOM GPU→CPU | REQ-CORE-002, REQ-ERR-002 | `src/backend/fallback.rs` |
...
| A4 | HW Fusion→Standalone（硬件约束违反时） | ARCH-4-FEATURES.md:1064,1275 | `gllm-kernels/src/compiler/hw_constraints.rs` |
| A5 | Reshape/Transpose 元数据 NOP | CLAUDE.md NO_SILENT_FALLBACK 例外 | `src/compat/gpu_compile.rs` |

### 🔴 未授权 Fallback — BUG（11 个，待修复）

| # | 文件:行 | Fallback 行为 | 违反规则 |
|---|--------|-------------|---------|
| B1 | `src/client.rs:303` | `detect_architecture().unwrap_or(ModelArchitecture::Llama4)` — 架构检测失败静默降级为 Llama4 | ARCH-ERR: 禁止 unwrap_or 掩盖错误 |
| B2 | `src/compat/cpu_backend.rs:131` | `get_system_memory_pressure().unwrap_or(0.0)` — 内存压力读取失败静默返回 0 | REQ-OBS-001: 禁止 memory_pressure 默认值 |
| B3 | `src/compat/cpu_backend.rs:209` | softmax sum=0 → uniform distribution — 数值异常静默降级 | ARCH-ERR: 禁止静默错误处理 |
| B4 | `src/compat/decoder_forward.rs:1363-1364` | `unwrap_or(9454)` / `unwrap_or(2753)` — rerank token ID 硬编码 fallback | ARCH-ERR: 禁止 unwrap_or 掩盖错误 |
| B5 | `src/compat/gpu_compile.rs:1481` | `_ => 9` — 未知 GPU platform 静默默认 Apple9 | ARCH-ERR: 禁止 catch-all 默认值 |
| B6 | `src/engine/executor.rs:540-544` | ONNX 无 KV cache 输出 → O(n²) 模式 — 静默性能降级 | ARCH-ERR: 禁止静默降级 |
| B7 | `src/engine/executor.rs:881` | `session_finalized_position().unwrap_or(0)` — session 位置查询失败静默归零 | ARCH-ERR: 禁止 unwrap_or 掩盖错误 |
| B8 | `src/model_config.rs:137` | tensor-driven config 失败 → gllm.config fallback | 无 SPEC 授权此 fallback 链 |
| B9 | `gllm-kernels/.../x86_64.rs:1098` | Paged attention (GPU-only) → naive attention on CPU | 无 SPEC 授权此降级 |
| B10 | `gllm-kernels/.../trace_emitter.rs:607` | SM < 70 → unified tiled GEMM — PTX codegen 静默降级 | REQ-KERNELS-PTX-MV-003: 禁止 SM 版本 fallback |
| B11 | `gllm-kernels/src/inference/backend.rs` | `CpuFallbackBackend` 调用 scalar_ops — 整个后端违反 NO_SCALAR | NO_SCALAR: 零容忍 |

### ❌ SPEC 明确禁止的 Fallback 模式（6 类）
