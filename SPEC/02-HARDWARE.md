...
| `tensor_core_gen` | u32 | Tensor Core 代数（0/1/2/3） | GEMM 策略选择（JitGpuTensorCore / JitGpu） |
...
12 个硬件 Profile，每个定义了核心能力和对融合拓扑的影响。
...
#### sm_70-79 (Volta/Turing)

- **核心能力**: wmma 16×16×16; 无异步内存; FP16 Tensor Core
- **拓扑影响**: wmma 精度受限; 寄存器压力限制融合深度; 无异步预取
...
- **核心能力**: 256-bit 统一 SIMD; 31 GPR; 基础 BF16/VNNI
- **拓扑影响**: 31 GPR 支持 6-8 ops epilogue; 无 VP2INTERSECT，sparse mask 走软件

#### AMX (AVX-512 + AMX)

- **核心能力**: 512-bit SIMD (32 zmm); VNNI; VP2INTERSECT; BF16; AMX tile 8×8 BF16
- **拓扑影响**: AMX tile GEMM 替代 BLIS 微内核; 32 zmm 无溢出支持 8-op epilogue

#### AVX-512

- **核心能力**: 512-bit SIMD (32 zmm); VNNI; BF16; 无 AMX
- **拓扑影响**: BLIS 微内核做 GEMM; epilogue 用 AVX-512
...
- **核心能力**: ZA Array 2D 存储; streaming SVE 模式; outer product 矩阵乘
- **拓扑影响**: ZA array 消除显式 tile 管理节点; outer product 直接做 attention

#### SVE2

- **核心能力**: 可变长 SIMD (128-2048 bit); SVE2 整数/浮点; 无 ZA array
- **拓扑影响**: 回退 BLIS 风格 GEMM; epilogue 用 SVE2 predicated 向量

#### NEON

- **核心能力**: 128-bit 固定 SIMD; ASIMD
- **拓扑影响**: 最保守: BLIS 微内核 + NEON epilogue; 类似 AVX2 体验
