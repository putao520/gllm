...
pub struct MegaKernelBusinessConfig {
    ...
    /// 启用后: embed 阶段检查 session_position > 0 → 跳过已处理 tokens
    pub session_enabled: bool,

    // ── Multimodal Fused Hidden 注入 (REQ-MEGA-MM-001) ──
    /// 是否编译 multimodal 模式 (预计算 fused hidden state 注入)
    /// 启用后: embed 阶段读取 fused_hidden_ptr → ADD 到 token embedding
...
embed 后, 层循环前:
  ...
  ├── 行为:
  │   ├── session_position > 0:
  │   │   ├── 跳过 embed 阶段: input_ids[0..session_position] 已在 KV cache
...
Phase 4.5: OutputModeDispatch (JMP table)            ← 新增
  ├── Load output_mode_selector from ABI 参数
...
```
MegaKernelFn 调用签名更新为 19 参数 (含 session + multimodal)
