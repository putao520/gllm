...
> 经融合管线自动推导。禁止在 `compile_mega_kernel_vm()` 中硬编码 VmInstr 序列。**
...
§0.2.5 虚拟维度   SymDim::Symbolic("seq_len")         运行时值读取 ([rbp+16])
  (SymDim)        Symbolic → StackArg offset           实际立即数或内存读取
...
Phase 2 R0: 瓶颈虚拟化 (PainPointAnalyzer → OpBottleneckMap) [§3.9]
  ...
  └── 输入: ModelProfile × DeviceProfile × InferenceMode → 纯静态计算
...
错误: compile_mega_kernel_vm() 手动 emit LoopBegin → emit_fusion_groups → emit Argmax → ...
正确: decoder_model() 构建完整图 → 7 轮虚拟化求解 → RegAlloc → Phase 3 物化
...
`compile_mega_kernel_vm()` **仅**负责 MegaKernelFn ABI 初始化（加载参数、计算派生值），
...
MegaKernelBusinessConfig {
  ...
  ↓ 传入图构建器
CompilerGraph::decoder_model(&config)
  ↓ 根据 config 决定图的形状
  ├── embed_gather
  ├── [if sg] SgInject
...
MegaKernelFn 扩展签名 (17 参数):
  fn(input_ids_ptr, weight_blob_ptr, kv_cache_ptr, positions_ptr,
     aux_ptr, batch_size,                                    ← register params (不变)
     prompt_len, scratchpad_ptr, output_tokens_ptr,
     temperature_u32, top_k, top_p_u32, max_new_tokens, eos_token_id,
     output_mode_selector,                                   ← 新增: [rbp+80] (原 hook_ctx_ptr 位置)
     hook_ctx_ptr,                                           ← [rbp+88] (原 telemetry_ptr 位置)
     telemetry_ptr)                                          ← [rbp+96] (新增尾部)
    → rax: 返回值语义随 output_mode 变化 (见下表)
...
返回值语义 (rax):
  ...
  ├── ClassifyBinary: 0 (2 个 f32 logits 已写入 output_tokens_ptr 作为 f32 指针)
  ├── ClassifyMultiway: 0 (N 个 f32 logits 已写入 output_tokens_ptr)
  └── EncodeToLayer:  0 (hidden_size 个 f32 已写入 output_tokens_ptr)
```

**ABI 演进策略**: `output_mode_selector` 插入到 `eos_token_id` 之后、`hook_ctx_ptr` 之前。
所有后续参数偏移 +8 bytes。旧调用方传 16 参数时，`hook_ctx_ptr` 落在 `[rbp+80]`
（新 ABI 的 `output_mode_selector`），值为 NULL (0) → 被解释为 Generate 模式 → 兼容。

**栈偏移表 (更新后)**:

```
[rbp+16]  prompt_len
[rbp+24]  scratchpad_ptr
[rbp+32]  output_tokens_ptr
[rbp+40]  temperature_u32
[rbp+48]  top_k
[rbp+56]  top_p_u32
[rbp+64]  max_new_tokens
[rbp+72]  eos_token_id
[rbp+80]  output_mode_selector  ← 新增 (原 hook_ctx_ptr 位置)
[rbp+88]  hook_ctx_ptr          ← 原 telemetry_ptr 位置
[rbp+96]  telemetry_ptr         ← 新增尾部
```

#### JMP Table 结构

```
compile_mega_kernel_vm() Phase 布局:

Phase 0: Load ABI params (17 个)
Phase 1: Compute derived values
Phase 2: Generate loop begin (LoopBegin)
Phase 3: Compute per-iteration input_ptr
Phase 4: Forward pass (embed → N layers → lm_head)  ← emit_fusion_groups
Phase 4.5: OutputModeDispatch (JMP table)            ← 新增
  ...
  └── CMP output_mode_selector, 3
      JE .encode_path
.generate_path:                           ← 现有 Phase 5-7 逻辑
  Argmax → StoreToken → CheckStopCondition
  JMP .loop_end                           ← 跳到 Phase 8
...
#### compile_mega_kernel_vm() Phase 4.5 伪代码
...
## §2 全模型图构建 (decoder_model)

### §2.1 图中算子

`CompilerGraph::decoder_model(business_config)` 根据 §1.5 业务配置构建模型图。
基础骨架 + 条件插入的业务 ops:
...
gllm 场景: decoder_model 单层模板 ≈ 20 ops, 全模型 ≈ 30 ops
...
### §3.8 compile_mega_kernel() 全链路

```
compile_mega_kernel(config)
  ├── 0. 性能建模 (Phase 1.5: R0)
  │     PerformanceModel::analyze(&graph, &device_profile)
  │     └── 每个 GEMM 的瓶颈分析 + 最优策略推荐
  ├── 1. CompilerGraph::decoder_model()
  │     └── embed + 单层模板(layer.前缀) + final_norm + lm_head + argmax + store + check
...
impl ModelProfile {
    /// 从模型配置推导每个 GEMM 的形状
    pub fn gemm_shapes(&self, mode: InferenceMode) -> Vec<(GemmRole, (usize, usize, usize))> {
        let m = match mode { Latency => 1, Throughput(seq) => seq };
        let hd = self.hidden_dim;
        vec![
            (QkvProjection,  (m, (self.num_heads + 2*self.num_kv_heads) * self.head_dim, hd)),
            (OutputProjection, (m, hd, self.num_heads * self.head_dim)),
            (GateUpProjection, (m, 2 * self.intermediate_size, hd)),
            (DownProjection,   (m, hd, self.intermediate_size)),
            (LmHead,           (m, self.vocab_size, hd)),
        ]
    }
}
...
impl PainPointAnalyzer {
    /// 纯静态分析: (ModelProfile × DeviceProfile × InferenceMode) → 痛点列表
    pub fn analyze(
        model: &ModelProfile,
        ...
        model.gemm_shapes(mode).into_iter().map(|(role, (m, n, k))| {
...
StoreToken 输入: token_id_ptr (VReg)
    ...
    ↓ 需要 output_tokens_ptr (MegaKernelFn ABI StackArg(32))
```
...
CheckStopCondition 输入: token_id_ptr (VReg)
    ↓ 需要 gen_loop_counter (AbiPtrs.gen_loop_counter)
    ↓ 需要 eos_token_id (StackArg(72))
    ↓ 需要 max_new_tokens (StackArg(64))
```
...
| Store seq_len=1 到 [rbp+96] | ABI 特定 |
...
| `mod.rs` | `compile_mega_kernel()` 传递 LayerLoopConfig + 多轮优化 |
...
```
新增第 17 参数: output_mode_selector: u32 → [rbp+80]
后续参数偏移:
  hook_ctx_ptr:     [rbp+80] → [rbp+88]
  telemetry_ptr:    [rbp+88] → [rbp+96]

MEGA_KERNEL_STACK_OFFSETS 新增尾部偏移
...
新增:
  OutputModeDispatch { selector: VRegId, paths: Vec<usize> }
    — Load [rbp+80] + CMP+JE 链分发到各路径

  BreakLoop { return_value: u32 }
    — 跳出 generate loop 到函数尾部，设置 rax=return_value
```

#### B.3 compile_mega_kernel_vm() Phase 4.5

在 Phase 4 (forward pass emit_fusion_groups) 之后:
1. 如果 `output_modes.len() > 1`: 发射 OutputModeDispatch + 各路径指令
2. 如果 `output_modes.len() == 1`: 走单一路径 (现有逻辑不变)
3. generate 路径: Argmax + StoreToken + CheckStopCondition (现有 Phase 5-7)
4. classify 路径: WriteLogits + BreakLoop(0)
5. encode 路径: Pool + BreakLoop(0)

#### B.4 x86 Lower (x86_lower.rs)

```
OutputModeDispatch lowering:
  mov REG, [rbp+80]        ; load selector
  cmp REG, 0
  je .generate_path
  cmp REG, 1
  je .classify_binary_path
  ...
  cmp REG, 3
  je .encode_path
  ; 默认走 generate (兼容旧 ABI selector=0)

BreakLoop lowering:
  mov rax, return_value
  jmp .function_epilogue   ; 跳过 LoopEnd 到 RET
```

#### B.5 gllm 侧 MegaKernelExecutor 更新

```
MegaKernelFn 调用签名更新为 17 参数
output_mode_selector 传入:
  generate → 0
  classify_binary → 1
  classify_multiway → 2
  encode_to_layer → 3
...
返回值语义:
  rax > 0 → generate: token 数量
  rax == 0 → classify/encode: output buffer 已填充
```
