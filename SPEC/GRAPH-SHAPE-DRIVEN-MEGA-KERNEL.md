...
### §1.5.5 与 HEAD-ROUTING "零重编译" 原则的兼容

> HEAD-ROUTING.md §2.2 铁律: **零权重重载、零 JIT 重编译**。

```
解决方案: 所有 output_modes 在一次编译中全部包含。

编译时:
  graph_builders.rs 根据 output_modes 构建多条尾部路径
  → 融合/规划/物化 一次性处理所有路径
  → 最终产物是一个 mega-kernel 函数, 包含所有输出模式的代码

运行时:
  MegaKernelFn ABI 新增参数: output_mode_selector: u32
  ├── 0 = Generate (默认)
  ├── 1 = ClassifyBinary
  ├── 2 = ClassifyMultiway
  └── 3 = EncodeToLayer

  在图尾部, output_mode_selector 驱动一个 JMP table:
  ├── final_norm → lm_head (共享, 所有模式都走)
  └── lm_head 后:
      CMP output_mode_selector, 0 → JE .generate_path (argmax→store→check→loop)
      CMP output_mode_selector, 1 → JE .classify_binary_path (write 2 logits)
      CMP output_mode_selector, 2 → JE .classify_multiway_path (write N logits)
      // EncodeToLayer 路径已由层循环内 EarlyExit 截断, 不到达此处
```
