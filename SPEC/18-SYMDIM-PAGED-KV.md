...
Generate Loop:
  ...
  embed → N × [QKV proj → attention → FFN] → lm_head → argmax → store token
...
`compile_forward_from_graph()` L239: `SYMDIM_MAX_SEQ_LEN` → 已有的 `max_seq_len` 参数（从 `geometry.max_seq_len` 传入）。
...
ABI 从 21 参数扩展为 22 参数。`page_table_ptr` 作为 arg 22，指向 `u32[]` page table（`page_table[seq_pos] = physical_page_id`）。当 `NULL` 时使用连续 stride（向后兼容）。GPU launch 同步扩展 args 数组。
