...
> P0-P3 全部完成的详细执行计划。每个任务标注影响文件、依赖关系、验证方法。
...
### P3.2 Audio Encoder (USM Conformer)
...
| 阶段 | 内容 | 任务数 | 核心复杂度 |
|------|------|--------|-----------|
| **P0** | 核心算子 + E2E 验证 | 6 | DualRoPE (中), 其余低 |
| **P1** | SharedKvRef + 融合 | 2 | SharedKvRef (**高**) |
| **P2** | PLE + E2B 验证 | 2 | 中 |
| **P3** | 多模态 | 3 | Vision/Audio encoder (高), 路由 (中) |

**里程碑**:
- P0.6 完成 → Gemma 4 31B GGUF 可用
- P2.2 完成 → Gemma 4 E2B 可用 (含 PLE + SharedKV)
