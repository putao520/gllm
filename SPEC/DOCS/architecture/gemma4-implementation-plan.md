...
>
> 状态图例: ✅ 已交付（有 commit 佐证）| 🟡 进行中（代码骨架就位、实现未完成）| ⏸ 未启动
...
| P1.1 SharedKvRef — Graph executor 层跳过 K/V MatMul | `decoder_forward::attention_step` 共享层读 donor | T43 (`32b6f10`) |
| P1.2 FusedQkvNormRope pattern_fusion 识别 (Q/K/V proj + QkNorm + ValueNorm + DualRoPE 六算子融合) | `pattern_fusion.rs::FusedQkvNormRopeRule` | T29 (`9697fb8`), T41/T42 (`dedd86d`) |
| P1.2 YAML ↔ atomic_op_to_kind 契约 | `atomic_op_to_kind` 覆盖 QkNorm/DualRoPE/Attention 派发；白名单移除 | T34/T35 (`ef38822`), T36 (`7dd6520`, `c0d0382`) |
...
|------|------|-----------------------|
| P2.1 PLE 算子链 (alias 解析 + fuzzy shape binding + YAML only_if 展开 + 5-input 签名一致性 + ColumnSlice JIT + GPU codegen) | ✅ 交付 | T28.2 (`6f6b648`), T28.3 (`774a51f`), T33 (`9dc283b`), T37 (`bab213c`), T38, T36 (`7dd6520`/`c0d0382`) |
| P2.2 E2B E2E 数值对齐 | 🟡 进行中 | dry-run ✅ 通 (T40)；剩余：真实 HF 下载 + 与 HF Transformers 参考输出余弦相似度 ≥ 0.99 验证 (T47) |
