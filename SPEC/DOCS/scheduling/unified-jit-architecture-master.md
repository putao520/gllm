...
## 10. 环境感知多版本算子：Epilogue 信号 → 编译期变体 → 运行时零开销选路 (ARCH-ENV-VARIANT)

> **核心法则**: 同一标量算子在不同运行时环境（硬件能力 × 负载特征 × 白嫖信号）下编译为多种物理实现，由 Mega-Kernel 块级路由在实现间零开销切换。
> **SSOT 交叉引用**: 02-ARCHITECTURE.md §17 (架构定义), 03-DATA-STRUCTURE.md §16 (数据结构), 07-OBSERVABILITY.md §8 (信号采集与路由), jit-cache-protocol.md §5 (缓存协议)

### 10.1 设计动机：§3 路由与 §4 白嫖的系统性闭环

§3 定义了 Mega-Kernel 块级路由——Thread Block 读取 `Request_State_Table` 决定走哪条路径。§4 定义了 Epilogue 白嫖——在算子尾段免费采集运行时特征（熵、质心、能量差等）。但两者之间缺少一个**系统性的桥梁**：

- §3 的路由决策基于"静态硬编码的条件判断"（如 `dead_neuron > 50%`）
- §4 的白嫖信号是连续 f32 值，无法直接驱动 O(1) 查表
- §6.1 已禁止"运行时 Amax 检测 → 精度切换"的 if-else 模式

**核心洞察**: 将 §4 的白嫖信号在 Epilogue 内就地**量化为离散枚举**（1 条 `vcmpps` + 1 条 `vpshufb` 查表），与硬件维度一起构成**编译期已知的笛卡尔积**。运行时只做表查找，零分支。

这就是 ARCH-ENV-VARIANT 的核心——将 §3、§4、§5 三个支柱统一为一个**"信号 → 编译 → 选路"**的闭环系统。

### 10.2 环境向量 (EnvVector)：§4 白嫖信号的离散化协议

运行时环境的 5 维离散化表示，每个字段为 2-3 个枚举值之一，bit-packed 为 `u32`：

```rust
pub struct EnvVector {
    pub sharpness: SharpnessBin,       // Softmax 锐度: Sharp / Medium / Flat
    pub dead_neuron: DeadNeuronBin,    // 死神经元密度: None / Low / High
    pub residual_energy: EnergyBin,    // 残差能量: Active / Dormant / Skip
...
| §4 白嫖信号 | 量化方式 | EnvVector 字段 | 写入时序 |
|------------|---------|---------------|---------|
| Softmax max/sum 比值 (§4.1, §13.9) | `vcmpps` + `vpshufb` LUT | `sharpness` | Softmax Epilogue |
| SiLU 死神经元计数 (§13.5) | `vpcmpd` + 计数 | `dead_neuron` | SiLU Epilogue |
| Δρ + cosθ 联合 (§13.3, §13.11) | 2×`vcmpps` + 交叉编码 | `residual_energy` | Residual Add Epilogue |
| 调度器 phase 标记 | 枚举直传 | `phase` | Batcher |
| prompt_len_skew | 统计 + 阈值比较 | `batch_shape` | Batcher |

**总开销**: ~10 条 SIMD 指令，~10 cycles，消耗 2-3 个寄存器。完全在 Epilogue 的空闲寄存器预算内（AVX-512 有 12 个空闲 zmm）。

### 10.3 与 §3 Mega-Kernel 块级路由的融合

§3.1 定义了 Thread Block 读取 `Request_State_Table` 的路由机制。EnvVector 直接嵌入该表：
...
pub struct RequestState {
    ...
    pub seq_len: u32,
    pub total_seq: u32,
    // 新增: 环境向量 (bit-packed u32)
    pub env_vector: u32,
    // bit 0-1: sharpness, bit 2-3: dead_neuron,
    // bit 4-5: residual_energy, bit 6: phase,
    // bit 7-8: batch_shape, bit 9-31: reserved
}
```

**Thread Block 选路流程（与 §3.1 融合）**:

```
Mega-Kernel 启动
  │
  ├─ Thread Block 检查 Request_State_Table (与 §3.1 一致)
  │
  ├─ 读取 env_vector 字段 (LDG)
  │
  ├─ 通过 bfe.u32 / shr + and 提取各字段
  │
  ├─ 查找完美哈希跳表 (§3.1 的内置字典) → VariantId
  │
  └─ jmp [jump_table + VariantId * 8] → 特化机器码入口
```

这与 §3.1 的路由机制完全一致——唯一的区别是路由键从"硬编码条件"升级为"EnvVector 离散枚举"。零分支预测惩罚，零额外 Kernel Launch。

### 10.4 与 §5 Hot JMP Patching 的融合

§5 定义了 JIT Director Daemon 的全局共识热修补机制。EnvVector 为其增加了**变体级生命周期管理**：

| §5 机制 | EnvVariant 扩展 | SPEC 位置 |
|---------|----------------|-----------|
| 卡尔曼迟滞监控 (§5.1) | 持续记录 EnvVector 分布直方图 | 02-ARCHITECTURE.md §17.8 |
| 静态键热替换 (§5.2) | 热变体提升为默认跳表入口 | jit-cache-protocol.md §5.4 |
| 冷专家封杀 (§6.4) | 冷变体 → Uncommon Trap 替代 | 02-ARCHITECTURE.md §17.8 |

**变体冷热分级**:

| 级别 | 条件 | 处理 |
|------|------|------|
| **热** | hit_rate > 10% | 默认跳表入口 |
| **温** | 1% < hit_rate ≤ 10% | LRU 缓存 |
| **冷** | hit_rate ≤ 1% | Uncommon Trap → §7.4 Deopt 恢复 |
| **僵尸** | 100K step 零命中 | 物理剔除，释放内存 |

**与 §7.4 的联动**: 当 Thread Block 撞入冷变体的 Uncommon Trap 时，触发与 §7.4 完全一致的 Deopt 流程：写入 `DEOPT_REQUEST` → 挂起 → Host 微冷冻 → 权重唤回 → 单独重放。

### 10.5 与 §2 TurboQuant 的融合

TurboQuant 的三个在线旋转插入点（§2.1）和双轨显存池（§2.2）直接影响变体编译：

| TurboQuant 特性 | 变体特化 | EnvVector 条件 |
|----------------|---------|---------------|
| Attention 后 FWHT (§2.1) | Softmax 变体选择是否内联 FWHT | sharpness=Flat |
| KV K per-channel scale (§2.2) | KV Write 变体选择 KIVI 路径 | sharpness=Medium/Sharp |
| Sink Token FP16 保护 (§2.2) | Attention 变体选择保护前 N token | sharpness=Sharp |
| RaBitQ ‖v‖ 修正 (§2.1) | RmsNorm 变体追加修正因子 | residual_energy=Dormant |

**核心原则**: TurboQuant 的数学优化不改变——变体系统只是在编译期为不同的环境组合预生成不同的优化组合。运行时选路是零开销的。

### 10.6 编译协议扩展

**编译时机**: 模型加载时（与 §2 TurboQuant 静态编译一致），不违反 §5 "推理热路径零编译"铁律。

**编译流程（扩展 §5.1 的 JIT Director）**:

```
模型加载:
  1. DeviceProfile::detect() → 硬件维度 (与 §1 一致)
  2. 枚举 EnvSchema 的合法 EnvVector 笛卡尔积:
     - 理论: 3 × 3 × 3 × 2 × 3 = 162 种
     - 剪枝后: 每个 OpKind 仅 4-8 个有效变体
  3. 对每个有效 (OpKind, EnvVector) 编译变体:
     - Phase 0-3 JIT 管线完整执行
     - SpecializationHint 驱动特化:
       * SinkProtection → Sink FP16 保护
       * KiviPerChannelScale → K per-channel 量化
       * FwhtRotation → 在线 FWHT 内联
       * GateCompaction → vcompress 挤压比
       * EarlyExit → NOP slide + Thread Exit
       * CentroidPrefetch → cuMemPrefetchAsync
       * RabitQCorrection → ‖v‖ 修正因子
  4. 注册到 EnvVariantRegistry (完美哈希表)
  5. 缓存: L1 注册表 + L2 共享代码页 + L3 磁盘
```
...
### 10.7 五大核心算子的环境感知变体示例

#### Softmax

| 变体 | 环境条件 | 与本蓝图 § 的联动 |
|------|---------|----------------|
| `Softmax_Prefill_Flat` | phase=Prefill, sharpness=Flat | 标准 FlashAttention tiled，无 Sink 保护 |
| `Softmax_Decode_Medium` | phase=Decode, sharpness=Medium | 标准 + Sink FP16 保护 (§2.2 KIVI) |
| `Softmax_Decode_Sharp` | phase=Decode, sharpness=Sharp | 质心精准预取 (§6.5) + Sink 保护 |
| `Softmax_Decode_Flat` | phase=Decode, sharpness=Flat | 激进量化 + FWHT 内联 (§2.1) |

#### RmsNorm

| 变体 | 环境条件 | 与本蓝图 § 的联动 |
|------|---------|----------------|
| `RmsNorm_IntoGemm` | 后继=GEMM, phase=Decode | NormIntoGemm 融合，零中间写回 |
| `RmsNorm_WithScale` | dead_neuron=High | 追加 per-channel scale (§4.1) → KIVI |
| `RmsNorm_WithRabitQ` | residual_energy=Dormant | 追加 ‖v‖ RaBitQ 修正 (§2.1) |

#### FFN (Gate/Up/Down)

| 变体 | 环境条件 | 与本蓝图 § 的联动 |
|------|---------|----------------|
| `FFN_FullDense` | dead_neuron=None | Gate+Up 共享 pack_a → 全融合 |
| `FFN_GateSkip_Low` | dead_neuron=Low | vcompress 挤压 20% 行 (§3.2, §6.2) |
| `FFN_GateSkip_High` | dead_neuron=High | vcompress 挤压 50%+ 行 (§6.2) |
| `FFN_ResidualSkip` | residual_energy=Skip | Thread Block 级路由跳过 (§6.3) |

#### Attention

| 变体 | 环境条件 | 与本蓝图 § 的联动 |
|------|---------|----------------|
| `Attn_Prefill` | phase=Prefill | FlashAttention tiled + DenseLocal KvView |
| `Attn_Decode_Medium` | phase=Decode, sharpness=Medium | PagedHeadMajor + Sink 保护 |
| `Attn_Decode_Sharp` | phase=Decode, sharpness=Sharp | Paged + 质心预取 (§6.5 RDMA/PCIe) |
| `Attn_Decode_Flat` | phase=Decode, sharpness=Flat | 激进量化 + FWHT (§2.1) |

#### KV Write

| 变体 | 环境条件 | 与本蓝图 § 的联动 |
|------|---------|----------------|
| `KV_Write_Plain` | sharpness=Flat | 标准 scatter 写入 |
| `KV_Write_KIVI` | sharpness=Medium | per-channel scale + K 量化 (§2.2) |
| `KV_Write_SinkProtect` | sharpness=Sharp | 前 N token FP16 + FWHT 旋转 (§2.1) |

### 10.8 全景闭环：§1-§9 + ARCH-ENV-VARIANT 的终极统一

```
┌─────────────────────────────────────────────────────────────────┐
│  gllm 大一统推理引擎 — 环境感知变体全景闭环                       │
│                                                                  │
│  §1 硬件探测 → DeviceProfile (编译期确定)                        │
│       │                                                          │
│       ▼                                                          │
│  §2 TurboQuant → QuantType × 数学优化 (编译期确定)               │
│       │                                                          │
│       ▼                                                          │
│  §10 ARCH-ENV-VARIANT (编译期)                                   │
│  ├── 枚举 (DeviceProfile × EnvSchema) 笛卡尔积                   │
│  ├── 对每个 (OpKind, EnvVector) 编译物理变体                     │
│  └── 注册到 EnvVariantRegistry (完美哈希表)                      │
│       │                                                          │
│       ▼                                                          │
│  §3 Mega-Kernel 块级路由 (运行时)                                │
│  └── Thread Block 读 env_vector → 跳表选 VariantId → jmp        │
│       │                                                          │
│       ├── §4 Epilogue 白嫖 (运行时, Mega-Kernel 内)              │
│       │   └── 采集 → 量化 → 写入 env_vector → 下游消费          │
│       │                                                          │
│       ├── §5 Hot JMP Patching (后台 JIT Director)                │
│       │   └── EnvVector 分布直方图 → 变体冷热分级                │
│       │       → 默认变体热修补 / 冷变体 Uncommon Trap            │
│       │                                                          │
│       ├── §7 MoE (运行时, Mega-Kernel 内)                        │
│       │   └── Expert 变体 = (ExpertId × EnvVector) 的注册表       │
│       │                                                          │
│       └── §8 残差总线 (运行时, Mega-Kernel 内)                   │
│           └── Early Exit 变体 = residual_energy=Skip 时触发      │
└─────────────────────────────────────────────────────────────────┘
```

**终极结论**: ARCH-ENV-VARIANT 不是孤立的新功能，它是将 §1 硬件探测、§2 TurboQuant、§3 Mega-Kernel 路由、§4 Epilogue 白嫖、§5 热修补这五大支柱**系统性地焊接**在一起的终极连接层。在这套闭环下：

- §3 的块级路由不再是"硬编码条件判断"，而是由 §4 白嫖信号驱动的**动态环境感知选路**
- §4 的白嫖信号不再是"一次性消费"，而是持续驱动**编译期生成的多版本算子选择**
- §5 的热修补不再仅限于"冷专家封杀"，还管理着**变体的冷热生命周期演化**
- §7 的 MoE 专家分发变成了 (ExpertId × EnvVector) 的**二维跳表查表**
- §8 的 Early Exit 变成了 residual_energy=Skip 时的**变体级 Thread Block 路由**

这就是 gllm "同一标量算子，多种不同甚至图级不同的推理实现"的完整工程化落地——**编译期变体爆发，运行期状态归零**。
