...
| `turbo_quant_bits` | usize | (新增) 强制下发的极化静态位宽 (如 4) |
...
pub struct ModelConfig {
    ...
    pub max_context: u64,
    pub rope_theta: f32,
    pub turbo_quant_bits: u8, // 固定引擎物理带宽
}
...
| `turbo_quant_bits` | 读取系统下发或模型配置 | 禁止动态浮点检测 |
...
| `turbo_quant_bits` | 强制极化位宽 | 从配置表映射 |
...
| `turbo_quant_bits` | 强制应用极化位宽常量 | 必须存在参数，否则报错 |
...
| `turbo_quant_bits` | 将 initializer 视作静态定点序列 | 强制使用极化参数 |
...
`ModelConfig.turbo_quant_bits` 是 JIT 翻译阶段能够感受到的唯一宽度常量，由引擎本身的能力和所分配的静态块决定。不再有运行期的 `dtype_size` 推理。

#### 位宽传播路径

所有的执行与编译节点皆强硬锁定在 `INT4` 甚至更低极化规格：

| 张量角色 | 计算通道 | 内存通道 | 传播约束 |
|-------------|-------|-------------|---------|
| GEMM weight | JIT 烧录静态寻址 | KV-Frozen 页表侧载 | 在 GPU Launch 前即通过 Kernel 模板宏固化 |
| Norm weight | JIT 融合缩放节点 | 同上 | 吸收进最终极化计算中 |
| Activation / intermediate | 静态开辟栈顶 | 伴随 Kernel 随开随毁 | **禁止**返回给 Host 端！ |
| KV cache | Paged Block (Dual-Track) | Native 4-bit / 8-bit | `gpu_alloc_kv_paged_pool(turbo_quant_bits)` |
...
| **极化位宽** | `TurboQuantBits` | 全局块的压扁维度分配 | W4A4 vs W8A8 |
...
pub enum TurboQuantBits {
    W4A4,  // 极化对齐的 4-bit 连续高密块
    W8A8,  // 极化对齐的 8-bit 高密块
}
```

TurboQuant 约束了 Codegen：

| 层面 | W4A4 | W8A8 |
|------|------|------|
| **内存寻址** | 纯 `byte_width` 游标 | 纯 `byte_width` 游标 |
| **向量加速** | 自动映射硬件整型内积 |自动映射硬件内积 |
...
**与 TurboQuant 的关系**: `QuantGemm` 源数据为极化整数，系统依赖 `TurboQuantBits` 维度在后端产生硬件原生整数乘加指令链，禁止执行浮点解包。
...
QuantGemm 强制内部映射 `TurboQuantBits` 发射硬件整型内置算符（如 `vpdpbusd` / `sdot`）
...
| `turbo_quant_bits` | `TurboQuantBits` | 极化块存储位宽 |
...
pub struct KvLayoutContract {
    ...
    pub turbo_quant: TurboQuantBits,
...
| 允许项 | 说明 |
|--------|------|
| metadata bridge | 统一 shape/turbo_quant/quant 语义 |
