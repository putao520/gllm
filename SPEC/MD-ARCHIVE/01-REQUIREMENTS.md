...
| **REQ-CORE-003** | 静态极化量化支持 (TurboQuant) | 支持统一强转极化格式 (INT4/FP8) | 1. 能够加载量化模型权重<br>2. 运行时支持多精度混合分派(BF16/F16/F32/I8/INT4 等),TurboQuant 作为其中一种固定块量化规格,具体精度按权重 dtype 与用户编译参数推导<br>3. 废除反量化，只提供定点/微浮点硬派算子 | 🟢 已实现 |
...
| **REQ-FUSION-DEEP-006** | 跨层残差融合 | Decoder layer 的残差 Add 与下一层的 RmsNorm 融合 | 1. Layer N 的 `residual_add` + Layer N+1 的 `RmsNorm` 融合为单 kernel<br>2. 消除层间中间张量写回（seq_len × hidden × 4 bytes）<br>3. Mega-Kernel 全层融合图构建支持跨层边 | 🟢 已实现 |

### 8.5 JIT 图执行通用化 (REQ-JIT-GRAPH)

> **背景**: 当前存在两个相互关联的问题：(1) `CompilerGraph` shape 在构建时硬编码为具体数值，`total_seq` 每步 +1 导致 CachedGQA 图每步重编译；(2) auto_graph 与实际执行路径断层，每新增模型都需手写分支。
...
| **REQ-JIT-GRAPH-003** | 图执行器打通（auto_graph → JIT 端到端） | `CompilerGraph`（从 auto_graph 构建）直接驱动 JIT 编译为 Mega-Kernel | 1. Mega-Kernel 编译后能执行完整 decoder forward（含 KV cache）<br>2. 新模型只需在 ARCH_TABLE 注册 + auto_graph 添加特征检测，不需要修改任何 Rust 代码<br>3. auto_graph → `CompilerGraph` → JIT 编译 → Mega-Kernel 执行链路端到端跑通<br>4. 现有手写分支（MoE、GQA）已全部迁移到 auto_graph → Mega-Kernel 路径<br>5. Mega-Kernel 支持 symbolic shape binding（依赖 REQ-JIT-GRAPH-001） | 🟢 已实现 |
...
| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-JIT-CACHE-001** | 全核心态级预编译 | 所有算子必须在模型加载阶段完成全网计算图预编译 | 1. 禁用独立算子的 JIT 缓存支持<br>2. `MegaKernelExecutor` 为唯一的编译和执行入口<br>3. 推理热路径中无任何单层或单算子重编译 | 🟢 已设定 |
...
| **REQ-JIT-CACHE-004** | 热路径零编译 | 推理 Decode Step 层循环中严禁用例 | `compile()` 调用频次必须恒为 0（推理热路径中不得编译，SPEC/39） | 🟢 已设定 |
...
| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-SG-001** | Level Keys 预计算 | 模型加载期通过小 CompilerGraph 预计算 3 个层级键向量（每个检测层） | 1. `LevelKeysCache` 对每个 `detection_depth × num_layers` 填充 3 个非全零 finite 向量；2. 向量维度 = `num_kv_heads × head_dim`；3. 小图通过 Mega-Kernel JIT 编译 / 运行（ARCH-FULL-JIT 合规）；4. 同一小图在 CPU/PTX/HIP/MSL 产出数值一致（容差 1e-4） | ✅ 已实现 |
| **REQ-SG-002** | FusedAttentionLayer Q-Tap | 检测层 `FusedAttentionLayer` 编译期注入 `q_tap`；JIT codegen 在 `q_proj` 尾段追加 STG 写 ring buffer | 1. `q_tap: None` 时零额外指令（cargo-asm diff 对比）；2. `q_tap: Some(...)` 时主计算结果与无 tap 版本一致（L2 误差 < 1e-4）；3. Ring buffer 读出 Q 与 CPU 参考 `q_proj(hidden)[-1]` 一致；4. 双缓冲 atomic step_index 协议防陈旧读；5. 未拆融合 / 未引入后端特化 GraphType | ✅ 已实现 — ring buffer 在 Executor::new() 预创建，sink_ptr/step_index_ptr 编入 JIT；SG 注册时复用同一 buffer [commit: gllm-kernels 777c3b52, gllm d6ec8c0] |
...
| **REQ-SG-008** | E2E 行为差异验证 | 注册 SG 后 mega-kernel callback table 路径端到端接通：JIT NativeCall → C ABI callback → vtable dispatch → KnowledgeProvider.retrieve() → TextEncoder.encode("Paris") → SgSharedMemory → SgInject (hidden += confidence × knowledge_vector) | 1. ✅ `FixedTextProvider("Paris")` → `retrieve()` 被 callback 调用 240 次 (NO_ISLAND_MODULE); 2. ✅ SG-on 输出与 baseline 不同 (`diff=true`) — SgInject 读取正确 SgSharedMemory 偏移 (confidence+12, knowledge_vector+16+hidden*4); 3. ✅ StackAlign + YMM save/restore + CLD 使嵌套调用 (tokenizer, JIT) 安全 | ✅ 已实现 [commit: gllm-kernels 75d2f3c8, gllm bed5660] |
...
| **REQ-HR-003** | Mid-layer encode | Mega-Kernel callback table 通过 `MidLayerEncodeCallback` 截断, `encode_to_layer` pool hidden 返回 | 1. `client.encode_to_layer(text, LayerAnchor::Relative(0.5), PoolMode::MeanPool)` 返回 `Ok(Vec<f32>)` 长度 = hidden_size；2. 向量所有元素 finite, L2 norm > 0；3. 不同 anchor (0.5 vs 0.9) 产生**不同** embedding — 证明截断深度真实生效；4. 禁止 stub / scalar fallback / silent Ok | 🟢 已实现 |
...
| **REQ-HR-005** | NO_ISLAND_MODULE 合规 | HR API 真实接入生产路径,禁止仅 `#[cfg(test)]` 调用 | 1. `grep classify_binary\|classify_multiway\|encode_to_layer src/*.rs` 在非 `#[cfg(test)]` 路径中有真实实现与导出；2. `Client::classify_binary` 真实调用 `MegaKernelExecutor::execute_score_tokens()` (output_mode=4)；3. E2E 测试 TEST-HR-001/002/004 通过真实 SmolLM2 推理路径验证 | 🟢 已实现 |
...
- `src/compat/cpu_backend.rs` — Backend trait 方法签名 (Executor 直接调 mega-kernel 绕过 Backend)
- `src/engine/mega_kernel.rs` — `execute_score_tokens()` (output_mode=4) + `execute_encode_at_layer()` (output_mode=5)
- `src/compat/mod.rs` — 相关 trait 方法签名
- `src/compat/gpu_backend_macro.rs` — GPU 后端 (CUDA/HIP/Metal) 统一返回 `Unimplemented` (显式,非 silent)
- `src/graph/executor.rs` — Mega-Kernel callback table 扩展 (Part 1 基础设施)
...
- `src/compat/cpu_backend.rs` — Backend trait 方法签名 (Executor 通过 callback_table_ptr 集成到 mega-kernel generate)
- `src/engine/mega_kernel.rs` — Mega-Kernel ABI arg 21 (callback_table_ptr) 传入 GuardrailProbeCallback
...
- `src/compat/cpu_backend.rs` — Backend trait 方法签名 (Executor 直接调 mega-kernel 绕过 Backend)
- `src/engine/mega_kernel.rs` — `execute_encode_at_layer()` (output_mode=5)
- `src/compat/gpu_backend_macro.rs` — GPU 后端 `Unimplemented` (显式)
- `src/lib.rs` — 公共导出
...
| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-COT-001** | Manual 模式 budget 精确控制 | 用户指定 `max_reasoning_tokens` + `step_count`，引擎严格遵守预算与步数上限 | 1. `reasoning_trace.len() ≤ step_count`；2. `total_reasoning_tokens ≤ max_reasoning_tokens`（允许 ≤10% tokenizer 估算误差）；3. `stopped_reason ∈ { StepCountReached, BudgetExhausted }`；4. 最终 `text` 非空 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-002** | Auto 模式 pattern-match 停止 | `ReasoningMode::Auto` 下任一 `stop_patterns` 子串在 chunk 中命中即停止 | 1. 命中时 `stopped_reason == PatternMatched(p)`；2. 未命中时 `stopped_reason ∈ { BudgetExhausted, EntropyConverged }`，不静默继续；3. `reasoning_trace.len() ≥ 1` | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-003** | Auto 模式 entropy-convergence 停止 | `entropy_threshold: Some(t)` 启用连续 chunk 文本熵估算收敛检测（当前启发式版本，logit 级见 §5.2 未来） | 1. 已知重复模式文本喂入 → 估算 entropy < t → `EntropyConverged`；2. 单元测试覆盖启发式本身；3. 文档说明真实 logit-level entropy 依赖 `GenerationResponse` 扩展 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-004** | Reasoning trace 完整保留 | 每个 reasoning step 产出的 chunk text 作为独立元素存入 `reasoning_trace: Vec<String>` | 1. `reasoning_trace.len() == actual_steps`；2. 每个 trace 元素非空；3. trace 中保留 step chunk 的原始语义内容 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-005** | 与 Semantic Gatekeeper 正交 | `register_semantic_gatekeeper(cfg)` 后调用 `reason(...)` 可正常工作，step 间的 `Client::generate` 自然走 SG 注入 | 1. `reason` 调用不 panic；2. 每次内部 `execute_generation` 触发 SG callback ≥1 次（mock counter 验证）；3. 不修改 SG API 或 Callback chain | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-006** | NO_ISLAND_MODULE 合规 | `Client::reason` 必须在真实 SDK 路径被调用，不是孤岛模块 | 1. `GenerationBuilder::reasoning` → `ReasoningBuilder::execute` → `Client::reason` 的转发链真实存在；2. E2E 测试 `test_cot_006_arbitrary_llm` 用 SmolLM2-135M-Instruct 跑通全链；3. grep `Client::reason` 在 `src/` 非测试代码中有至少 1 个真实调用点 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-007** | Step Hook trait 定义与生命周期 | `ReasoningStepHook` trait 提供 `on_step_start` / `on_step_end` 回调，通过 `ReasoningBuilder::with_step_hook()` 注册 | 1. trait 有 `on_step_start(&mut self, &StepContext) -> StepAction` + `on_step_end(&mut self, &StepResult) -> StepKnowledge` 两个方法；2. trait bound 包含 `Send + Sync`；3. E2E 测试: 注册 hook 后 Manual 3 步 reasoning，`on_step_start` 和 `on_step_end` 各被调用 3 次；4. 不注册 hook 时行为与原实现完全一致 (backward compatible) | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-008** | StepContext 包含 accumulated reasoning text | `StepContext` 传递前序步骤累积文本，允许 hook 基于历史做动态决策 | 1. `on_step_start` 的 `StepContext.accumulated_text` 在 step_index=0 时为空串；2. step_index=1 时包含 step 0 的 chunk_text；3. step_index=2 时包含 step 0+1 的累积文本；4. `model_name` 非空；5. `remaining_budget` > 0 且逐步递减 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
| **REQ-COT-009** | StepAction 支持 Continue/Skip/InjectPrompt/Halt | `StepAction` 枚举提供 4 种流程控制，`StepKnowledge` 支持跨步知识注入与温度覆盖 | 1. `Continue` 正常执行；2. `Skip` 跳过某步且 trace 无空元素；3. `InjectPrompt(extra)` 的 extra 出现在该步 prompt 中；4. `Halt(reason)` 提前终止且 `stopped_reason = HaltByHook(reason)`；5. E2E 测试覆盖 `InjectPrompt`: 注入关键词后 chunk 包含该关键词 | 🟢 已实现 [cot_reasoner.rs + E2E tests] |
...
- `Client::reason(prompt, mode, template)` 在 `src/cot_reasoner.rs` 实现，内部多轮调用 `Client::execute_generation` 复用现有 Mega-Kernel JIT 缓存
...
| **REQ-MEGA-SG-002** | Mega-Kernel Callback Table | mega-kernel 通过 ABI arg 20 传递 C 风格函数指针表，JIT 在 SgDetect 后 CALL 外部回调函数实现知识注入 | 1. ✅ MegaKernelCallbackTable (#[repr(C)], 8 slots × 16B)<br>2. ✅ Slot 0 = SG_KNOWLEDGE_RETRIEVE: 回调读 detect_hidden → KnowledgeProvider.retrieve() → TextEncoder.encode() → 写 knowledge_vector + confidence<br>3. ✅ VmInstr::NativeCall + LoadCallbackEntry x86_64 ISA lowering (caller-saved regs save/restore, call rax)<br>4. ✅ SgDetect 尾部 +CALL callback (LoadCallbackEntry → GprSkipIfNull → MemFence Release → NativeCall → MemFence Acquire) + SgInject SIMD FMA 注入 (hidden += confidence × knowledge_vector)<br>5. ✅ callback_table_ptr=NULL 时 GprSkipIfNull 零开销<br>6. ✅ Executor::callback_table 字段，set_sg_callback_shim 注册 slot 0，clear_sg_callback_shim 清理 | ✅ 已实现 [commit: gllm 55acd87 + 2c92244, gllm-kernels 489157cb] |
...
## 21. 统一图来源：Mega-Kernel 唯一执行路径 (REQ-UGS)

> **SSOT**: [gllm-kernels SPEC/GRAPH-SHAPE-DRIVEN-MEGA-KERNEL.md §2.4-2.6]
> **背景**: gllm 历史上有两条编译/执行路径：(A) FusedGraph 路径 (auto_graph→OnnxGraph→FusedGraph(FusedOp)→CompilerGraph(OpKind)→JIT) 和 (B) Mega-Kernel 路径 (auto_graph→CompilerGraph(OpKind)→JIT)。路径 A 的 FusedOp→OpKind 转换存在系统性字段丢失（RoPE.partial_ratio 硬编码 1.0、FlashAttention.scale 丢失、GQA.num_kv_heads 丢失、MoERouting.top_k 丢失），且路径 A 的逐层编译无法支持 generate loop 融合。
>
> **架构决策 — Mega-Kernel 为唯一路径**：删除 FusedGraph/FusedOp/FusedGraphExecutor 及其关联优化器 (pattern_fusion/hardware_fusion/constant_folding/dead_code)，删除 `to_onnx_graph()` 中间表示。Mega-Kernel 是系统唯一的编译和执行路径，所有模型（dense + MoE）统一使用。
>
> **唯一数据流**: `auto_graph` (tensor-name-driven) → `CompilerGraph` → `compile()` → 单次 CALL → 推理结果

| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-UGS-001** | auto_graph→CompilerGraph 直接构建器 | `auto_graph` 从 tensor names 直接构建 CompilerGraph（跳过 OnnxGraph/FusedGraph 中间层） | 1. `auto_graph` 根据 tensor names 推导架构特征，直接构建 CompilerGraph<br>2. `op_type` 字符串 → `OpKind` 枚举完整映射表（Gather→OpKind::Gather, MatMul→OpKind::Gemm, SimplifiedLayerNormalization→OpKind::RmsNorm, SiLU→OpKind::Silu, Mul→OpKind::Mul, Add→OpKind::Add, RotaryEmbedding→OpKind::RoPE, Attention→OpKind::MultiHeadAttention, QkNorm→OpKind::QkNorm, ValueNorm→OpKind::ValueNorm, PerLayerEmbed→OpKind::PerLayerEmbed, PleSlice→OpKind::ColumnSlice, DualRotaryEmbedding→展开为两个 OpKind::RoPE）<br>3. 模型配置属性 → `OpKind` 字段注入（theta→RoPE.theta, eps→RmsNorm.eps, head_dim→QkNorm.head_dim 等）<br>4. `only_if` 条件守卫复用 `eval_only_if_with_loop()` 逻辑<br>5. `repeat` 块展开为 N 组独立 OpKind 节点<br>6. mega-kernel 独有节点（SgInject, SessionKvRestore, MmHiddenInject, GenerateLoop）由 `MegaKernelBusinessConfig` 驱动注入<br>7. 所有已支持架构（Llama/Qwen3/Gemma4/Phi4/Mistral/DeepSeek）构建结果与 `decoder_model()` / `decoder_model_hetero()` 等价 | 🟢 已实现 [SmolLM2 E2E 2026-04-27] |
| **REQ-UGS-002** | 异构层模板折叠 | `to_compiler_graph()` 根据 `config.attention_pattern[]` + `config.num_kv_shared_layers` 自动生成异构层图，替代手写 `decoder_model_hetero()` | 1. 按 `(attention_pattern[i], ffn_size[i])` 二元组去重，推导 K 个唯一层模板（如 Gemma-4 E2B → 4 模板: sliding_small/full_small/sliding_large/full_large）<br>2. 权重布局自动推导：从模板中收集所有权重张量，按模板分组计算 `PerLayerWeightLayout`，替代手写 `compute_per_layer_bytes`<br>3. `HeteroLayerLoopConfig` 从 `config.attention_pattern[]` + `config.intermediate_size[]` 自动生成，包含 `num_segments` 和 `layers_per_segment`<br>4. SharedKvRef 条件分支：层模板中 k_proj/k_norm/v_proj/v_norm/rope_k 五个 op 前插入 `GprCondAction { condition: layer_idx < threshold, action: Skip(N) }`。Consumer 层运行时跳过这五个 op（省 GEMM 算力），MHA 从 donor 层 KV cache page 读取 K/V（见 SPEC 03 §1.3.1）<br>5. Consumer 层 MHA 的 KV 输入通过 PagedAttention 间接寻址指向 donor 层 KV cache page，`effective_kv_layer()` 按 attention 类型匹配 donor（同类型最后一个非共享层），无需额外 tensor 引用<br>6. SharedKvRef Prefill 两阶段加速（见 SPEC 03 §1.3.2, SPEC 32 §2.4）：Donor pass 完成后 donor KV 驻留 L2 → Consumer pass MHA 读命中 L2；节省 `num_shared × 2` 次 K/V GEMM（~30% prefill FLOPs）；非 SharedKvRef 退化为单阶段零开销 | ✅ 条件分支已实现 [GprCondAction + LayerCondition + CmpGeU, 2026-06-04]; K 模板折叠 + prefill 两阶段为性能优化待后续 |
| **REQ-UGS-003** | 删除 graph_builders.rs 手写图构建 | `MegaKernelExecutor` 消费 auto_graph 构建的 CompilerGraph，`InferenceCompiler::compile() (SPEC/39 统一编译入口)` 接受外部 CompilerGraph | 1. `MegaKernelExecutor` 调用 auto_graph 构建 CompilerGraph + `compile() (SPEC/39 统一编译入口)`<br>2. `gllm-kernels` 新增 `compile() (SPEC/39 统一编译入口)` 接受外部 CompilerGraph<br>3. `gllm-kernels/src/compiler/graph_builders.rs` 中 `decoder_model()` / `decoder_model_hetero()` / `build_layer_body()` 函数保留（旧路径仍可用）<br>4. 所有 E2E 测试通过（SmolLM2, Gemma-4 E2B）<br>5. 新增模型只需在 ARCH_TABLE 注册 + auto_graph 添加特征检测 + 标量算子注册，零额外 Rust 代码修改 | 🟢 已实现 [SmolLM2 E2E 2026-04-27] |
...
| **REQ-UGS-005** | 删除 FusedGraph/FusedOp 整套子系统 | 物理删除 FusedGraph、FusedOp、FusedNode、所有 Config 结构体、FusedGraphExecutor、build_*_graph 函数、graph/optimizer/ 整个目录 | 1. `src/graph/types.rs` 中 FusedOp 枚举（含全部 25+ 变体）、FusedNode、FusedGraph、FlashAttentionConfig、SwiGLUConfig、RoPEConfig、FusedQkvRopeConfig、FusedQkvNormRopeConfig、FusedRMSLinearConfig、GQAConfig、MoERoutingConfig、PleConfig、AtomicOp 全部删除<br>2. `src/graph/executor.rs` 中 FusedGraphExecutor 结构体、build_flash_attention_graph、build_swiglu_graph、build_rope_graph、build_fused_qkv_rope_graph、build_fused_qkv_norm_rope_graph、build_node_graph_inner（FusedOp→OpKind 转换中心）全部删除<br>3. `src/graph/optimizer/pattern_fusion.rs`、`hardware_fusion.rs`、`constant_folding.rs`、`dead_code.rs`、`pass.rs` 全部删除<br>4. `src/engine/executor.rs` 中 `graph_executor: Option<FusedGraphExecutor>` 字段、`graph_executor_ptr: *mut FusedGraphExecutor` 字段、所有 `graph_executor_ptr` 赋值、逐节点 JIT 执行路径全部删除<br>5. `src/compat/` 中所有引用 FusedGraphExecutor 的代码路径删除<br>6. 编译通过（`cargo check`）+ 所有非 MoE E2E 测试通过 | 🟢 已实现 |
| **REQ-UGS-006** | MoE 模型 Mega-Kernel 路径 | MoE 模型（GPT-OSS-20B、DeepSeek-V3、GLM-5）走 Mega-Kernel 编译路径，删除 executor 中 `if is_moe { None }` 跳过逻辑 | 1. `executor.rs` 中删除 `if is_moe { None }` 分支，MoE 模型正常进入 mega-kernel 编译<br>2. MoE auto_graph 包含 MoEGate/MoERouter/MoEDispatchPacked OpKind 节点<br>3. `compile()` 正确处理 MoE OpKind（不再返回空 vec）<br>4. 专家权重布局：mega-kernel 权重打包支持 per-expert 权重 stride<br>5. MoE 模型 E2E 测试通过（GPT-OSS-20B 输出非退化） | 🟢 已实现 |
| **REQ-UGS-007** | 删除 to_onnx_graph() 中间表示 | 删除 `ArchTemplate::to_onnx_graph()` 方法及 OnnxGraph 中间表示，auto_graph 是唯一的图构建出口 | 1. `src/arch/template.rs` 整个文件删除（已被 auto_graph 替代）<br>2. `src/graph/types.rs` 中 OnnxGraph 相关类型保留（仍被 loader 等使用），FusedGraph 相关类型随 REQ-UGS-005 删除<br>3. 所有调用 `to_onnx_graph()` 的代码改为 auto_graph 直接构建<br>4. 编译通过 + E2E 测试通过 | 🟢 已实现 |
