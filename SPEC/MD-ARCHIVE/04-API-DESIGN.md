...
let pipeline_client = Client::builder()
    ...
    // SchedulingMode 从 batch_size 自动推导 (SPEC/12)，无需显式指定
    .build()?;
```
...
// 当前阶段: Err(HeadRoutingError::MidLayerNotSupported) - Mega-Kernel
// 单次前向未暴露 callback 截断路径,显式拒绝以保持 NO_SILENT_FALLBACK 合规。
// 落地依赖 Mega-Kernel callback table 扩展 mid-layer exit 支持。
```
...
pub enum HeadRoutingError {
    ...
    /// encode_to_layer 通过 Mega-Kernel callback table 的 MidLayerEncodeCallback 实现
...
1. **零重载**: `classify_binary` / `classify_multiway` 复用加载时的 Mega-Kernel,不触发权重重装或 JIT 重编译。多次调用之间 `Arc::as_ptr(&client.state().backend)` 地址恒定。
...
6. **Mid-layer encode 实现**: `encode_to_layer` 通过 `MidLayerEncodeCallback` + Mega-Kernel callback table 在 anchor 层触发 `ExitEarly { logits: hidden_state_as_f32 }`,由 Client 层按 `PoolMode` pool 后返回 `Vec<f32>`。不同 anchor / pool 产出不同向量,证明截断真实生效。
...
> **类型定义 SSOT**: `RetrieveContext`、`AstContext`、`AstSentinel`、`KnowledgeEntry` 的完整字段语义见 `SEMANTIC-GATEKEEPER.md §6`。此处仅列出 API 层精简签名。
...
│   2. compile(graph, config) ×2 (小图 JIT 编译)      │
│   3. 对每个 detection_depth:                        │
│        预计算 [K_L1, K_L2, K_L3] 存入 LevelKeysCache│
│   4. 为检测层 Attention OpKind 注入 q_tap 配置     │
│      (通过 MegaKernelBusinessConfig 驱动)           │
...
通过 Mega-Kernel JIT 编译执行两个小 `CompilerGraph`：
...
- `SemanticGatekeeperCallback.pre_node` 在 Mega-Kernel callback table 中真实被调用（通过 SgDetect/SgInject 共享内存，非 `#[cfg(test)]` 路径）
