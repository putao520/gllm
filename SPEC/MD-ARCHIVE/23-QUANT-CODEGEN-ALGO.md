...
Gather 不做 dot-product，语义是"从量化表中读出行并还原为 compute_dtype hidden state"。compute_dtype 由 TensorMeta.dtype × DeviceProfile 推导，量化路径计算精度从权重 dtype 推导(多精度混合支持 F32/BF16/F16) 累加后输出，原生浮点路径保持原始精度：
