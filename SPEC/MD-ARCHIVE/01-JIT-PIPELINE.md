...
>
> 交叉引用: `37-HARDWARE-ACCELERATION.md` §3 定义了 DynamicPrecisionSelect TraceOp 的完整语义和 auto_select 映射。

## 0. CompilerGraph 来源 (ARCH-UNIFIED-GRAPH-SOURCE)

**铁律：CompilerGraph 的唯一来源是 gllm 侧 auto_graph 系统。gllm-kernels 不包含独立的图构建逻辑。**

```
gllm: config.json → ModelConfig → ResolvedConfig
    ↓
gllm: auto_graph (tensor-name-driven) → CompilerGraph
    ↓
...
- ❌ 新增模型时修改 gllm-kernels 代码（只需在 ARCH_TABLE 注册 token + auto_graph 添加特征检测 + 标量算子注册）
...
| ConditionalBranch | VecConditional | auto_lower_trace | ✅ auto_select.rs 已实现 |
...
| ConditionalBranch | 条件执行路径, SelectOp 替代 | VecConditional | ✅ auto_select.rs 已实现 |
| QuantFma | 混合精度 GEMM (TurboQuant) | 专用 path via StrategySelector | ❌ 走专用 lower |
| BlockScale | 块缩放 (CDNA4 mfma_scale) | 专用 path | ❌ 走专用 lower |
| Prefetch | 质心预取, 权重预热 | PrefetchInstr | ✅ 占位 (ISA lowering 层 emit prefetch) |
| NonTemporalStore | KV 连续写入绕缓存 | NonTemporalStoreInstr | ✅ 占位 (ISA lowering 层 emit nontemporal) |
| BitExtract | 量化解包 (nibbles→f32) | BitExtractInstr | ✅ Error (量化解包用 QuantBlockLoad) |
| Permute | 向量重排 (shuffle, RoPE) | VecPermute | ✅ 已实现 (VecShuffle) |
| MaskedOp | 尾部掩码, 不对齐访问 | MaskedLoad/Store | ✅ 已实现 (ConditionalSelect) |
| AtomicAdd | MoE expert 命中计数 | AtomicAddU32 | ✅ 已实现 (VmInstr::AtomicAdd) |
| FWHT | TurboQuant 在线旋转 | butterfly 序列 | ✅ 已实现 (VecShuffle+Add/Sub+ConditionalSelect) |
...
fn select_gemm_strategy(
    ...
    match (hw.gpu_arch(), quant_a, quant_b) {
        ...
        // ── GPU 标准 tensor core / MFMA 路径 — 参数由 DeviceProfile 属性推导 ──
        (Sm100, BF16, BF16)
            => Sm100Tcgen05 { tile: hw.gemm_tile_sizes() },
        (Sm90, BF16 | F16, BF16 | F16)
            => Sm90WgmmaTma { num_waves: hw.optimal_waves(), cluster_dims: hw.optimal_cluster() },
        (Sm80, BF16 | F16, BF16 | F16)
            => Sm80MmaAsync { stages: hw.smem_stages(), split_k: 1 },
        (Sm70, F16, F16)
            => Sm70Wmma { stages: hw.smem_stages() },
        (Gfx12, F16, F16)
            => Rdna4WmmaF16 { tile: hw.gemm_tile_sizes() },
        (Gfx12, BF16, BF16)
            => Rdna4WmmaBf16 { tile: hw.gemm_tile_sizes() },
        (Gfx11, F16, F16)
            => Rdna3WmmaF16 { tile: hw.gemm_tile_sizes() },
        (Gfx11, BF16, BF16)
            => Rdna3WmmaBf16 { tile: hw.gemm_tile_sizes() },
        (Gfx942, BF16, BF16)
            => Cdna3MfmaBf16 { tile: hw.gemm_tile_sizes() },
        (Gfx942, F16, F16)
            => Cdna3MfmaF16 { tile: hw.gemm_tile_sizes() },
        (Gfx90A, BF16, BF16)
            => Cdna2MfmaBf16 { tile: hw.gemm_tile_sizes() },
        (Gfx90A, F16, F16)
            => Cdna2MfmaF16 { tile: hw.gemm_tile_sizes() },

        // ── Apple GPU 路径 — tile 参数由 DeviceProfile 属性推导 ──
        (AppleGpu, BF16, BF16) if hw.apple_gpu_gen() >= 9
            => MetalSimdMmBf16 { tile: hw.gemm_tile_sizes() },
        (AppleGpu, F16, F16) if hw.apple_gpu_gen() >= 8
            => MetalSimdMM { tile: hw.gemm_tile_sizes() },
        (AppleGpu, F32, F32) if hw.apple_gpu_gen() >= 7
            => MetalTiled { tile: hw.gemm_tile_sizes() },

        // ── CPU 量化路径 — 由参数化微核处理，dot_product_cap 决定计算指令 ──
        (_, qa, F32) if qa.is_quantized() => FusedQuantGemm {
            quant_type: qa,
            dot_cap: hw.dot_product_cap(),
            tile: hw.gemm_tile_sizes(),
        },

        // ── CPU 标准路径 — tile 参数由 DeviceProfile.gemm_tile_sizes() 推导 ──
        (_, F32, F32) if hw.has_amx()          => AmxTile { tile: hw.gemm_tile_sizes() },
        (_, BF16, F32) if hw.has_avx512_bf16() => Avx512Bf16 { tile: hw.gemm_tile_sizes() },
        (_, F32, F32) if hw.has_avx512()        => Avx512F32 { tile: hw.gemm_tile_sizes() },
        (_, F32, F32) if hw.has_avx2()          => Avx2Fma { tile: hw.gemm_tile_sizes() },
        (_, F32, F32) if hw.has_neon()          => NeonFmla { tile: hw.gemm_tile_sizes() },
        (_, F32, F32) if hw.has_sve()           => SveFmla { tile: hw.gemm_tile_sizes() },
...
| **可扩展** | 新增硬件变体只需在 `DeviceProfile` 中注册能力属性（`gemm_tile_sizes()`、`smem_stages()`、`optimal_waves()` 等），策略选择和 codegen 自动适配，无需新增 enum variant 或 match arm |
...
fn emit_group(group: &GroupStrategy, emitter: &mut impl MachineCodeEmitter) {
    match &group.compute {
        ...
        ComputeStrategy::Gemm(Avx2Fma { tile }) => emit_avx2_fma_gemm(emitter, tile),
        ComputeStrategy::DequantCompute(FusedQuantGemm { quant_type, dot_cap, tile }) =>
            emit_fused_quant_gemm(emitter, quant_type, dot_cap, tile),
...
- `InferenceCompiler::new()` / `compile()` / `build_*_graph()` 只在模型加载时调用
...
`auto_graph` 直接构建 CompilerGraph（OpKind 序列），传入 gllm-kernels 编译为 Mega-Kernel。

```
gllm: auto_graph (tensor-name-driven) → CompilerGraph (OpKind 序列)
gllm-kernels: compile(graph, config) → 单一 Mega-Kernel 机器码
```
...
`auto_graph` 根据 tensor names 推导架构特征，直接构建 OpKind 序列。OpKind 定义在 `gllm-kernels/src/compiler/graph.rs`，完整清单见 `SPEC/04-OPERATORS.md`。

**关键 OpKind 的子图展开**（由 auto_graph 在构建时直接生成，无中间 FusedGraph 层）：

| OpKind | 子图组成 |
|--------|---------|
| `MultiHeadAttention` | RmsNorm → Gemm(Wq) → Gemm(Wk) → Gemm(Wv) → RoPE(Q) → RoPE(K) → Attention(Q,K,V) → Gemm(Wo) |
| FFN (SwiGLU) | Gemm(gate) → Silu → Gemm(up) → Mul → Gemm(down) |
| `MoERouter` + `MoEDispatchPacked` | Gemm(gate) → Softmax → TopK → [专家分支] → Add(聚合) |
| `Gather` | 索引查找（embedding lookup），JIT 编译为循环内索引加载 |
| `SliceView` | 零拷贝视图（指针偏移），JIT NOP（已授权 SPEC 例外） |
| `PerLayerEmbed` | Slice(ple_weight) → Linear → Add |
| `QkNorm` | L2 归一化 + √head_dim 缩放 |
| `ValueNorm` | RMSNorm（无学习参数） |
