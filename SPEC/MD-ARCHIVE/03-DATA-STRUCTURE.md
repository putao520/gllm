...
量化 tensor 不经过 `Backend::upload_weights()` 上传，而是以原始 block bytes 存储在 `WeightsHandle.quantized` HashMap 中，JIT 编译时直接生成读取量化 block 的硬件原生 GEMM 机器码，禁止独立解包。
...
3. **架构识别** 只看 `general.architecture` 字符串, 禁止按 key 前缀或文件名
   ...
   `ModelGeometry::from_config` / auto_graph 中声明对应字段语义。
...
### 8.0 KvPageHeader (56B) — KV 页元数据 SSOT

> **📌 SSOT**: 本节是 `KvPageHeader` 物理布局的唯一真源。
> 19-KV-CACHE-OPTIMIZATION §2.1 定义 Epilogue 遥测和量化元数据语义。
> 22-PAGE-COMPRESSION §2 定义压缩区域语义。两者引用本节布局，不重复定义。

```
KvPageHeader (56B) 物理布局:
  ┌─ 基础区域 (8B)
  │   ├── page_id: u32              (4B) 页唯一标识
  │   └── ref_count: u32            (4B) 引用计数（跨 sequence 共享）
  │
  ├─ Epilogue 遥测区域 (16B) — 语义见 19-KV-CACHE-OPTIMIZATION §2.1
  │   ├── entropy_avg: f16          (2B) Softmax Epilogue: 注意力分散度
  │   ├── centroid_pos: f16         (2B) Softmax Epilogue: 注意力重心位置
  │   ├── softmax_max_avg: f16      (2B) Softmax Epilogue: 注意力峰值
  │   ├── delta_rho_avg: f16        (2B) Residual Epilogue: 跨层能量差
  │   ├── dead_ratio: u8            (1B) FFN Epilogue: 死神经元比例
  │   ├── importance_score: u8       (1B) 综合重要性评分 (0-255)
  │   ├── head_entropy_max: u8       (1B) per-head 最大 entropy
  │   └── head_entropy_min: u8       (1B) per-head 最小 entropy
  │
  ├─ 量化元数据区域 (12B) — 语义见 19-KV-CACHE-OPTIMIZATION §2.1
  │   ├── precision_tier: u8         (1B) 精度等级 (FP16/FP8/KIVI4/KIVI2/Sparse)
  │   ├── sink_mask: u32             (4B) page 内 sink token 位掩码
  │   ├── channel_bitmap_lo: u32     (4B) 通道稀疏掩码低 32 位 (MUSTAFAR)
  │   ├── k_scale_offset: u16        (2B) per-channel K scale 在页内偏移
  │   └── v_scale_factor: u8         (1B) per-token V scale 指数
  │
  ├─ 调度元数据区域 (8B) — 语义见 19-KV-CACHE-OPTIMIZATION §2.1
  │   ├── pipeline_id: u8            (1B) 所属管线 (Conversation/Working)
  │   ├── layer_mask: u32            (4B) 有效层位掩码（跨层共享页时标识）
  │   ├── tier_age: u16              (2B) 精度等级赋值后的 tick 计数
  │   └── deopt_flags: u8            (1B) Deopt 标志
  │
  ├─ 压缩区域 (8B) — 语义见 22-PAGE-COMPRESSION §2
  │   ├── codec: u8                  (1B) CompressionCodec (None/Lz4/BitPackRle/NvcompAns/ZstdDict)
  │   ├── compressed_size: u32       (4B) 压缩后字节数（0 = 未压缩）
  │   ├── checksum: u16              (2B) CRC16 校验（防腐）
  │   └── storage_tier: u8           (1B) StorageTier (GpuHbm/CpuDram/Nvme)
  │
  └─ 对齐填充 (4B)
