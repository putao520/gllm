...
|-------------|-------------|---------------------|-----------------|
| AVX2 | 32 | > 24 KB | hidden ≤ 6K elem (compute_dtype=F32 时) |
| AVX-512 | 32 | > 24 KB | hidden ≤ 6K elem (compute_dtype=F32 时) |
| NEON | 32-64 | > 24-48 KB | hidden ≤ 6-12K elem (compute_dtype=F32 时) |
| GPU sm_80 | 48-164 (SMEM) | > SMEM × 75% | tile ≤ 36-123 KB |
| GPU sm_90 | 228 (group SMEM) | > 171 KB | tile ≤ 171 KB |

> **注**: elem 数量阈值 = L1 × 75% / elem_bytes(compute_dtype)。BF16 模型 elem_bytes=2 时阈值翻倍。
