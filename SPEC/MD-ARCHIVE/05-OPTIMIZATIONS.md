...
所有需要在中间层介入的优化模块通过 Mega-Kernel callback table（`hook_ctx_ptr` + `callback_table_ptr` ABI 参数）接入 JIT 机器码内部执行。

### 1.1 CallbackAction

```rust
// E: Element — 元素类型匹配 compute_dtype (由 TensorMeta.dtype × DeviceProfile 推导)
// 量化模型: E = f32 (dequant 后累加); BF16 模型 + AVX-512: E = f32 (VDPBF16PS 累加)
// 未来 BF16 residual path: E = bf16
pub enum CallbackAction<E: Element> {
    /// 继续正常执行当前节点
    Continue,
    /// 跳过当前节点（Gate Skip / Dead Code Elimination）
    SkipThisNode,
    /// 提前终止层循环（Early Exit / Guardrail Veto）
    ExitEarly { logits: Option<Vec<E>> },
    /// 注入外部隐藏状态（RAG / Knowledge Injection）
    InjectHidden { data: Vec<E> },
}
```
...
```rust
pub struct LayerContext<'a, E: Element> {
    /// 当前 hidden_state 的可变引用 (dtype = compute_dtype)
    pub hidden_state: &'a mut [E],
    /// 当前层号 (0-based)
    pub layer_idx: usize,
    /// 模型总层数
...
```rust
pub trait LayerCallback<E: Element> {
    /// 节点执行前介入
    fn pre_node(&mut self, ctx: &LayerContext<E>, node_idx: usize) -> CallbackAction<E>;
    /// 节点执行后介入（遥测、探针）
    fn post_node(&mut self, ctx: &LayerContext<E>, node_idx: usize, output: &[E]);
}
...
### 1.4 Mega-Kernel Callback Table 接入点

```
Mega-Kernel CALL（JIT 机器码内部执行所有层）
  SgDetect / GuardrailProbe 通过 callback table (NativeCall) 触发
    → callback 返回 CallbackAction:
        Continue     → 正常执行
        SkipThisNode → 跳过当前层
        ExitEarly    → 立即退出
        InjectHidden → 替换 hidden_state
...
**关键技术**:
- **Level Keys 预计算** 通过小 `CompilerGraph`（EmbedLookupOnlyGraph + KProjOnlyGraph）走 `compile()` 编译执行（ARCH-FULL-JIT + ARCH-CPU-GPU-UNIFIED 合规）
- **Q 截获** 通过检测层 `MultiHeadAttention` OpKind 的 Q-tap epilogue 变体（复用 `§5` Epilogue 白嫖网络基础设施，不拆融合）
...
```rust
// E: Element — 权重元素类型, 匹配模型 TensorMeta.dtype (通常 F32 或 BF16)
pub struct DraftAdapter<E: Element> {
    config: AdapterConfig,
    weight: Arc<Vec<E>>,                // 共享 lm_head.weight [vocab_size, hidden_size], dtype = model dtype
    residual_delta: Option<Vec<E>>,     // Phase B: 蒸馏残差 [vocab_size, hidden_size], dtype 同上
    distillation_step: usize,           // 已完成的蒸馏步数
    norm_weight: Arc<Vec<E>>,           // 共享最后一层 norm 权重 [hidden_size], dtype = model dtype
}
```
...
自动化覆盖率审计确保所有 auto_graph 支持的算子都有完整的 JIT 管线支持：

```
1. 扫描 auto_graph 中使用的所有 OpKind → 提取所有算子类型
2. 对每个 OpKind 检查:
...
# 输出:
# [PASS] qwen3: 18/18 ops fully covered
# [PASS] llama4: 17/17 ops fully covered
# [FAIL] gpt-oss-20b: MoERouter missing codegen lowering
# [PASS] gemma4: 23/23 ops fully covered
```

**铁律**: 新增架构或新增 OpKind 变体时，覆盖率审计必须通过才能合入主分支。任何 `❌` 状态等同于未实现。
