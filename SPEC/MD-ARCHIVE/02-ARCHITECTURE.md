...
- **状态**: 🟢 已修复 — `split_phi4_qkv` 函数已在架构重构中移除，Phi4 现在走 auto_graph tensor-name-driven 驱动，维度从 `ModelConfig` 动态读取
...
**实现位置**: `auto_graph` 构建时直接生成 `CompilerGraph`，Attention 节点由 `OpKind::MultiHeadAttention` 表达，JIT 管线 Phase 2 Fusion Engine 处理融合。
...
- **注入点**: Mega-Kernel 内部 embedding lookup(Gather op 之后,第一个 transformer 层之前)
...
| Decoder-side fusion(已接入) | Mega-Kernel JIT 内部融合 |
...
│  量化: JIT 内核直接读取量化权重块，编译时生成硬件原生量化 GEMM 机器码。  │
│        TurboQuant FWHT 旋转矩阵作为编译时常量嵌入，无运行时反量化。      │
└─────────────────────────────────────────────────────────────────────────┘
```
...
client.embeddings(["text"]).generate()
    ...
    → MegaKernelExecutor 单次 CALL 进入预编译 Mega-Kernel
...
├── arch/               # 架构自动检测 (tensor-name-driven auto_graph)
├── graph/              # DAG 优化器 (统一表示)
...
> **状态**: ✅ 已实现 (REQ-GPU-001~012 全部完成，见 SPEC/15-GPU-HOST-GLUE.md)
...
GPU mega-kernel 的编译过程与 CPU 共享相同的 auto_graph → CompilerGraph → Phase 0-3 管线。
...
运行时 (Rust 不参与):
  ...
  kernel 内部完成: 融合组序列（由图拓扑推导, SPEC/39 §1.3.2）→ 采样 → token→text → 循环
...
> **实现状态**: 🟢 IMPLEMENTED — 11 fusion points codegen 完成 (x86_64 AVX2/AVX-512)
...
## §17 分布式通信集成架构 (ARCH-DISTRIBUTED-COMM)

> **关联 SPEC**: gllm-nccl SPEC/02-ARCHITECTURE.md §5 (集成接口), SPEC/26-VMINSTR-RATIONALIZATION.md §1.6 (通信 VmInstr)
> **feature gate**: `nccl` — 编译时门控，未启用时通信指令不存在于 VmInstr 枚举
> **依赖方向**: gllm-kernels → depends on → gllm-nccl（通过 `nccl` feature 门控）
...
gllm-nccl 是独立的跨设备集合通信库，使用 raw pointer API（`*mut u8`），不依赖 gllm-kernels 的类型系统。gllm-kernels 通过 `nccl` feature 编译时引入 gllm-nccl，在 JIT codegen 中生成通信指令的调用桩。

**权责边界**:

| 组件 | 所有权 | 职责 |
|------|--------|------|
| VmInstr 通信变体定义 | gllm-kernels SPEC/26 | 指令集定义 + ISA Lowering 规则 |
| 通信运行时实现 | gllm-nccl | AllReduce/AllGather/ReduceScatter/Broadcast 实现 |
| JIT 调用桩生成 | gllm-kernels | codegen 生成 `call` 指令链接到 gllm-nccl 运行时 |
| Compute-Comm 重叠编排 | gllm-kernels | JIT 生成的 mega-kernel 在 compute/comm stream 上并发 |

### 17.2 Feature 传播机制

```toml
# gllm-kernels/Cargo.toml
[features]
default = []
nccl = ["dep:gllm-nccl"]
cuda = ["gllm-nccl?/cuda"]
...
[dependencies]
gllm-nccl = { path = "../gllm-nccl", optional = true }
```

| Feature 组合 | 语义 |
|-------------|------|
| `--features cuda` | 单机 CUDA 计算，无通信 |
| `--features "cuda,nccl"` | CUDA 计算 + 分布式通信（gllm-nccl 以 cuda 后端编译）|
| `--features "hip,nccl"` | HIP 计算 + 分布式通信（gllm-nccl 以 hip 后端编译）|

### 17.3 Raw Pointer 接口契约

gllm-kernels 调用 gllm-nccl 时通过 raw pointer 转换，不共享类型：

```rust
// gllm-kernels 内部调用（gllm-kernels 侧代码）
#[cfg(feature = "nccl")]
{
    use gllm_nccl::CommHandle;
    let future = comm_handle.all_reduce(
        gpu_buf.as_device_ptr() as *const u8,  // GpuBuffer → raw ptr
        recv_buf.as_device_ptr() as *mut u8,
        count, DType::Fp16, ReduceOp::Sum,
    )?;
}
```

### 17.4 Compute-Comm Overlap 数据流

JIT 生成的融合 kernel 在 compute stream 和 comm stream 上并发：

```
Time ─────────────────────────────────────────────►

GPU Stream (compute):  [chunk_0 GEMM] [chunk_1 GEMM] [chunk_2 GEMM] ...
GPU Stream (comm):                  [chunk_0 AR]   [chunk_1 AR]   [chunk_2 AR] ...
                                     ↑ overlap ↑    ↑ overlap ↑

gllm-kernels scheduler:
  1. 发起 chunk_0 计算 (CUDA stream compute)
  2. chunk_0 计算完成 → 发起 chunk_0 AllReduce (CUDA stream comm)
  3. 同时发起 chunk_1 计算 (CUDA stream compute)
  4. chunk_0 AllReduce 完成 + chunk_1 计算完成 → 发起 chunk_1 AllReduce
  5. 重复...
```

gllm-kernels 负责 stream 编排，gllm-nccl 只负责在指定 stream 上执行通信操作。

### 17.5 VmInstr 通信指令

通信 VmInstr 定义在 SPEC/26 §1.6，共 3 个变体：

| VmInstr | 用途 | GPU Lowering | CPU Lowering |
|---------|------|-------------|-------------|
| `AllReduceChunk` | chunk 级 AllReduce | PTX/HIP call stub → gllm-nccl runtime | FFI call stub |
| `CommBarrier` | 通信/计算同步 | PTX `bar.sync` / HIP barrier | pthread_barrier |
| `NvlinkAsyncCopy` | NVLink 异步块拷贝 | PTX `cp.async.bulk` | NOP |

JIT codegen 遇到通信 VmInstr 时：
1. 生成调用桩（call stub），链接到 gllm-nccl 提供的运行时函数
2. 不内联通信实现（通信是跨设备操作，不能 JIT 内联）
3. 调用桩的参数通过系统 ABI 寄存器传递

### 17.6 与 gllm-nccl SPEC 的交叉对齐

| 本节 | 对齐 gllm-nccl SPEC |
|------|---------------------|
| §17.1 集成定位 | gllm-nccl SPEC/02 §5.1 集成架构 |
| §17.2 Feature 传播 | gllm-nccl SPEC/02 §5.4 Feature 传播 |
| §17.3 Raw Pointer | gllm-nccl SPEC/02 §5.2 Raw Pointer 接口契约 |
| §17.4 Overlap | gllm-nccl SPEC/05 §6.3 Compute-Comm Overlap |
| §17.5 VmInstr | gllm-nccl SPEC/07 §5 优化路径 AllReduce |
