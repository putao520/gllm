...
### 2.4.1 权重零拷贝直传 + 可选 compute_dtype 转换 (ARCH-WEIGHT-ZERO-COPY)

**核心原则**: "什么格式就跑什么格式" — Loader 不做任何格式转换，权重保持原始格式直传 JIT。

**双路径**:

| 路径 | 触发条件 | 行为 |
|------|---------|------|
| **零拷贝直传**（默认） | `compute_dtype == dtype` | 权重 raw bytes 原样传入 JIT，零转换 |
| **dtype 转换**（用户覆盖） | `compute_dtype != dtype` | dequantize → convert 到目标 dtype |

```rust
// Loader API
loader.with_compute_dtype(DType::FP8E4M3)  // 用户主动覆盖
```
...
| 格式 | Loader 行为 | JIT 消费 |
|------|------------|---------|
| Q4_0/Q4_1/Q5_0/Q5_1/Q8_0 | raw block bytes 直传 | `kquant_matmul` 原生内核 |
| Q2K/Q3K/Q4K/Q5K/Q6K/Q8K | raw block bytes 直传 | `kquant_matmul` K-Quant 内核 |
| IQ 系列 | raw block bytes 直传 | `kquant_matmul` IQ 内核 |
| MXFP4 | blocks + scales 分离直传 | `mxfp4 GEMM` 内核 |

**SafeTensors 原生精度零拷贝直传**:

| 格式 | Loader 行为 | JIT 消费 |
|------|------------|---------|
| BF16 | raw bytes 直传 | BF16 GEMM (VDPBF16PS) |
| F16 | raw bytes 直传 | F16 GEMM |
| F32 | raw bytes 直传 | F32 GEMM |

用户通过 `Loader::with_compute_dtype()` 可在加载时指定不同计算精度，
此时走路径 B（dequantize → convert）。默认走路径 A（零拷贝直传）。
...
- 诊断日志（标记 MXFP4 张量对）
- `mxfp4_scale_map()` 查询（auto_graph 绑定参考）
...
典型 decoder 图: `Gather → [LayerLoop: Norm → Q/K/V → HeadRmsNorm? → RoPE? → MHA → O_proj → Residual → PostNorm → FFN → Residual] → FinalNorm → Gemm(lm_head) → OutputMode`（融合组序列由图拓扑推导, SPEC/39 §1.3.2）
...
## 9. GGUF Tokenizer 自主解析 (REQ-TOK-GGUF)

> **问题**: GGUF 文件内嵌完整的 tokenizer 元数据（`tokenizer.ggml.*`），但 `TokenizerHandle` 只支持从 `tokenizer.json` 文件加载。纯 GGUF 模型（无外部 tokenizer.json）无法加载。
> **方案**: 直接解析 `tokenizer.ggml.*` 元数据，用 `tokenizers` crate 的 programmatic API 构建 `Tokenizer`。

### 9.1 GGUF Tokenizer 元数据字段 (REQ-TOK-GGUF-001)

| 字段 | 类型 | 含义 |
|------|------|------|
| `tokenizer.ggml.model` | STRING | 模型类型标识: `llama`/`gpt2`=BPE, `t5`/`whisper`/`albert`=Unigram |
| `tokenizer.ggml.tokens` | ARRAY[STRING] | 词表 token 字符串（索引 = token ID） |
| `tokenizer.ggml.scores` | ARRAY[FLOAT] | token log 概率分数（Unigram 核心，BPE 可选） |
| `tokenizer.ggml.token_type` | ARRAY[UINT] | token 类型: 0=normal, 1=unknown, 2=control, 3=user, 4=unused, 5=byte |
| `tokenizer.ggml.merges` | ARRAY[STRING] | BPE 合并规则（仅 BPE 模型存在） |
| `tokenizer.ggml.bos_token_id` | UINT | BOS token ID |
| `tokenizer.ggml.eos_token_id` | UINT | EOS token ID |
| `tokenizer.ggml.unknown_token_id` | UINT | UNK token ID |
| `tokenizer.ggml.padding_token_id` | UINT | PAD token ID（可选） |
| `tokenizer.ggml.add_bos_token` | BOOL | 编码时是否自动前置 BOS |
| `tokenizer.ggml.add_eos_token` | BOOL | 编码时是否自动追加 EOS |
| `tokenizer.chat_template` | STRING | Jinja2 chat 模板（可选） |

### 9.2 模型类型 → tokenizers crate 映射 (REQ-TOK-GGUF-002)

| `tokenizer.ggml.model` | tokenizers 模型 | 构造方式 |
|------------------------|----------------|---------|
| `llama`, `gpt2` | `BPE` | `HashMap<String, u32>` vocab + `Vec<(String, String)>` merges |
| `t5`, `whisper`, `albert`, `japanese` | `Unigram` | `Vec<(String, f64)>` vocab_scores |
| `rwkv` | `WordLevel` | `HashMap<String, u32>` vocab |

### 9.3 架构特定默认管道

| 架构族 | PreTokenizer | Normalizer | Decoder |
|--------|-------------|-----------|---------|
| llama/gemma (SentencePiece) | `MetaSpace` (replacement=▁) | `NFKC` | `MetaSpace` + `Fuse` |
| gpt2/qwen (ByteLevel) | `ByteLevel` | `NFC` | `ByteLevel` |
| t5 (SentencePiece+NFKC) | `MetaSpace` (replacement=▁) | `NFKC` + `Strip` | `MetaSpace` + `Fuse` |

### 9.4 TokenizerHandle 扩展 API (REQ-TOK-GGUF-003)

```rust
impl TokenizerHandle {
    /// 从 GGUF 内嵌 tokenizer 元数据直接构建 Tokenizer
    pub fn from_gguf_reader(reader: &GgufReader, model_kind: ModelKind) -> TokenizerResult<Self>
}
```

**集成路径**: `TokenizerHandle::from_loader()` 修改为：
1. `tokenizer_path()` 存在 → `from_path()`（现有路径）
2. `tokenizer_path()` 不存在 + GGUF reader 有 `tokenizer.ggml.tokens` → `from_gguf_reader()`（新路径）
3. 都不满足 → `TokenizerError::MissingTokenizer`

### 9.5 GgufReader 新增方法

```rust
impl GgufReader {
    pub fn tokenizer_model(&self) -> Option<&str>;           // tokenizer.ggml.model
    pub fn tokenizer_merges(&self) -> Result<Vec<&str>>;     // tokenizer.ggml.merges
    pub fn tokenizer_unknown_token_id(&self) -> Option<u32>; // tokenizer.ggml.unknown_token_id
    pub fn tokenizer_padding_token_id(&self) -> Option<u32>; // tokenizer.ggml.padding_token_id
    pub fn chat_template(&self) -> Option<&str>;             // tokenizer.chat_template
}
```
