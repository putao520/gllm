...
>
> 交叉引用: `01-JIT-PIPELINE.md` §5 (StrategySelector)、`02-HARDWARE.md` (DeviceProfile)、`04-OPERATORS.md` (算子库)、`37-HARDWARE-ACCELERATION.md` (硬件加速技术完整 PTX 指令、VmInstr 映射和 DeviceProfile 条件，本文档提供硬件指令能力矩阵总览)
...
> FA 版本命名 SSOT 见 16-DEVICE-FUSION.md §"GPU Attention 算法映射"。

| 世代 | Shared Memory | Attention Tile | Softmax | KV 读取 |
|------|--------------|---------------|---------|--------|
| SM80 (FA-v2) | 手动 cp.async | block_q=64, block_k=64 | online softmax (max+sum) | 分 stage 异步预取 |
| SM90 (FA-v3) | TMA bulk copy | block_q=128, block_k=128 | online softmax + swizzle | TMA 单线程发起 |
