...
> **SSOT 声明**: 本文档定义 SymDim 从 CompilerGraph 到 JIT 机器码的完整穿透机制。
...
gllm: build_node_graph()
  └─ CompilerGraph 中 OpKind 的 m/seq_len 维度 = SymDim::Symbolic("seq_len", max=2048)
      │
      ▼
gllm-kernels: compile() → plan_lower::lower_fusion_plan()
  ...
  └─ Phase 3: plan_lower → lower.rs → VmInstr
      ...
      │
      ▼
gllm: Mega-Kernel CALL (executor 传入 ShapeBinding)
...
let sym_slot_map = SymDimSlotMap::mega_kernel_abi();  // 预定义 seq_len→StackArg(16) 等
