...
| 任务 | 依赖 | 涉及子系统 | 说明 |
|------|------|-----------|------|
| T1 Multinomial 采样 | 无 | JIT codegen | 采样算子 JIT 生成 |
| T2 Thinking Budget | 无 | API + ABI | 参数扩展 + 栈参数传递 |
| T3 Thinking Token 标记 | T2 | JIT 内部状态机 | 编译期状态转换逻辑 |
| T4 Thinking KV 跳过 | T3 | JIT 内部 KV 管理 | 推理期 KV cache 跳过策略 |
| S1 EESD 接线 | 无 | Variant 编译 + dispatch | 浅层变体编译路径 |
| S2 EAGLE | S1 | Draft head variant | EAGLE draft head 编译 |
| S3 MTP | S1 | MTP head variant | MTP multi-token projection |
