...
> XlmR 和 XlmRNext 共用 xlmr canonical name，算法路径完全一致。
...
| 问题 | 详情 | 状态 |
|------|------|------|
| GPT-OSS (gpt-oss-20b) auto_graph 特征检测 | 真实 gpt-oss 架构 (MoE + sliding/full + yarn RoPE + RMSNorm + SiLU + bias + mxfp4) 需在 auto_graph 中添加特征检测 | ✅ MoE + attention_bias 特征检测 + YaRN RoPE scaling 传播已完成 |
