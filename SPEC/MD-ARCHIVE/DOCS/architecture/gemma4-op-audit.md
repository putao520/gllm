...
**涉及子系统**: 核心 RoPE 计算（复用），新增层类型分发 + partial 跳过逻辑。
...
**涉及子系统**: 新的权重加载路径 + 新的残差注入点。31B/26B 不需要此算子。
...
> P0-P3 表示技术依赖层级（非优先级排序），上层完成后下层才可开始。

| 层级 | 项目 | 影响范围 | 涉及子系统 | 依赖 |
|------|------|---------|-----------|------|
| P0 | `DualRotaryEmbedding` | 所有 Gemma 4 模型 | 核心 RoPE 计算 | 现有 RotaryEmbedding |
| P0 | `QkNorm` | 所有 Gemma 4 模型 | ElemWise 算子 | 无 |
| P0 | `ValueNorm` | 所有 Gemma 4 模型 | RMSNorm 子集 | 现有 RMSNorm |
| P0 | Attention per-layer type | 所有 Gemma 4 模型 | Attention 派发 | 现有 Attention |
| P0 | config.json 解析 | 所有模型 | model_config.rs | — |
| P1 | `SharedKvRef` | E2B/E4B | 调度器+KV cache+权重加载 | PagedScheduler |
| P1 | `FusedQkvNormRope` 融合 | 性能优化 | pattern fusion | P0 算子全部完成 |
| P2 | `PerLayerEmbedding` | 仅 E2B/E4B | 权重加载+残差注入 | 权重加载 |
| P3 | 多模态 (Vision/Audio) | 完整多模态 | encoder+routing+fusion | P0+P1 完成 |
