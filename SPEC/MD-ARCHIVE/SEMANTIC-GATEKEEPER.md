...
Semantic Gatekeeper（以下简称 **SG**）是 gllm 的**运行时可插拔 Callback 模块**，通过 Mega-Kernel callback table（`SgDetect` / `SgInject` 共享内存 + `NativeCall` VmInstr）在 JIT 机器码内部执行，对模型参数、算子图结构、权重布局**完全零修改**。
...
┌────────────────────────── 推理阶段（每 decode step） ─────────────────────┐
│                                                                              │
│  Mega-Kernel 单次 CALL（JIT 机器码内部）:                                     │
│  SgDetect 检测到 detection layer → callback table NativeCall:                │
│    ┌─ is_detection_layer(layer_idx)?                                         │
│    │    ├─ NO  → 正常执行，不触发 SG                                         │
...
两个图在模型加载期通过 `compile()` 编译一次，其 `CompiledLayer` 与主模型 `CompiledLayer` 并列常驻 `SemanticGatekeeperState`。
...
`auto_graph` 构建期，`is_detection_layer(layer_idx)` 的 `MultiHeadAttention` OpKind 节点才携带 `q_tap: Some(...)`。其他层 `q_tap: None`，完全零额外指令开销。

> **NO_ISLAND_MODULE 合规**: `q_tap` 字段必须在主推理路径（非测试代码）被 `auto_graph` 构建函数真实写入，且 Mega-Kernel `SgDetect` 必须真实读取 ring buffer。集成测试验证 Q 向量与 CPU 参考实现数值一致。
...
| 路径 | SG 实现 | 通信机制 |
|------|---------|---------|
| **batch_forward_gpu_pure** (仅 GPU macro 路径,Executor 已绕过) | Rust `LayerCallback::pre_node()` | Rust `CallbackChain::dispatch_pre_node()` |
...
| 2 | Reserved | —（CoT 不使用 mega-kernel callback，纯 Client 编排） | — |
...
| Mega-Kernel 执行模型 | `SPEC/03-GRAPH-IR.md §4` |
