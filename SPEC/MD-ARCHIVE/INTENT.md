...
```
 ...
 │     ├─ Mega-Kernel CALL (output_mode_selector │
 │     │    = EncodeToLayer(anchor_layer))       │
 │     │    在 anchor_layer 截断输出             │
...
| Backend CPU 路径 | Mega-Kernel CALL（`output_mode_selector` = EncodeToLayer，在 anchor 层截断） |
...
| `MegaKernelExecutor::execute_encode_at_layer()` (output_mode=5) | `src/engine/mega_kernel.rs` |
| Mega-Kernel CALL (output_mode_selector) | `src/engine/executor.rs` |
| 公共导出 | `src/lib.rs` |
| E2E 测试 | `tests/test_e2e_intent.rs` |
