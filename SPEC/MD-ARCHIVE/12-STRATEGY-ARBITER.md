...
> **SSOT 声明**: 本文档是 gllm 推理引擎全局优化策略优先级决策的唯一真源。定义 SchedulingMode（从运行时 batch_size 自然推导的调度模式）、GraphProfile（模型图拓扑特征提取）、StrategyArbiter（策略优先级仲裁），及其与 HwOptEngine（`02-HARDWARE.md §10`）的集成关系。
...
1. **使用场景盲区**: 同一硬件上，单请求低延迟推理与高吞吐批处理的最优策略完全相反，但 HwOptEngine 无法区分——而这一区分本质上由运行时 batch_size 决定：batch_size=1 是 memory-bound GEMV，batch_size>1 是 compute-bound GEMM。
...
```
batch_size (运行时值，调度器自然推导)
  └→ SchedulingMode (从 batch_size 推导: ≤1 → Latency, >1 → Throughput)
       └→ GraphProfiler (模型图静态分析)
            └→ StrategyArbiter (SchedulingMode × GraphProfile × DeviceProfile → StrategyBias)
                 └→ HwOptEngine (StrategyBias 注入 CostModel → HwOptPlan)
                      └→ JIT Codegen (生成机器码)
```

## 2. SchedulingMode — 调度模式（从 batch_size 推导）

### 2.1 核心设计：运行时 batch_size 自然推导

**InferenceMode 已被消灭。** 不存在用户侧 `.inference_mode()` API。调度模式从运行时 batch_size 自然推导：

- **batch_size ≤ 1** → 等效 Latency 模式（M=1 GEMV, memory-bound）
- **batch_size > 1** → 等效 Throughput 模式（M=batch_size GEMM, compute-bound）

编译器不区分模式——JIT 代码支持任意 M（`SymDim::Symbolic("batch_size")`），运行时自然落入不同 Roofline 区域。batch_size 是运行时值，不影响编译产物，因此 JIT 缓存 key 不包含模式信息。

```rust
/// 从运行时 batch_size 推导的调度模式。
/// 不存在用户侧 API——由调度器在每次调度决策时自动推导。
pub enum SchedulingMode {
    /// batch_size ≤ 1: 极致单请求延迟，所有资源服务唯一请求。
    /// GEMM 形态: M=1 GEMV（memory-bound），算术强度 ≈ 2。
    Latency,

    /// batch_size > 1: 极致吞吐，最大化 tokens/second/dollar。
    /// GEMM 形态: M=batch_size GEMM（compute-bound），算术强度随 M 增长。
    Throughput { batch_size: usize },
}

impl SchedulingMode {
    /// 从运行时 batch_size 推导调度模式。唯一合法的构造方式。
    pub fn from_batch_size(batch_size: usize) -> Self {
        if batch_size <= 1 {
            SchedulingMode::Latency
        } else {
            SchedulingMode::Throughput { batch_size }
        }
    }

    /// 获取用于 Roofline 分析的 effective_M。
    pub fn effective_m(&self) -> usize {
        match self {
            SchedulingMode::Latency => 1,
            SchedulingMode::Throughput { batch_size } => *batch_size,
        }
    }
}
```
...
**不存在 `.inference_mode()` Builder 方法。** 调度模式完全由运行时 batch_size 决定，无需用户指定。

```rust
let client = Client::builder()
    .model("Qwen/Qwen3-7B-Instruct")
    // 无 .inference_mode() —— 调度模式从 batch_size 自然推导
    .build()?;
```

**推导时机**: 每次调度决策时，调度器将当前 batch_size 传入 `SchedulingMode::from_batch_size()`，StrategyArbiter 据此生成对应的 StrategyBias。batch_size 变化时 StrategyBias 自然变化，无需重新加载模型。

### 2.3 模式影响矩阵

| 维度 | Latency (batch≤1) | Throughput (batch>1) |
|------|---------|------------|
| **Batch 上限** | 1（自然） | 动态（由 BatchSolver 求解） |
...
同一模型、同一硬件，不同 batch_size 的算术强度分布截然不同：

```
GEMM Arithmetic Intensity:
  AI = 2MNK / ((MK + KN + MN) × elem_bytes)

  batch_size=1  (M=1):  AI ≈ 2N/(K+N+1) ≈ 2    → MemoryBound
  batch_size=64 (M=64): AI ≈ 128NK/(64K+KN+64N) → ComputeBound
```

**REQ-ARB-001**: RooflineAnalyzer 必须接收 `batch_size` 以确定 M 值参与 AI 计算。batch_size ≤ 1 时 `effective_M = 1`；batch_size > 1 时 `effective_M = batch_size`。
...
接收 `SchedulingMode` × `GraphArchetype` × `DeviceProfile`，输出 `StrategyBias`——一组成本模型调制系数，注入 HwOptEngine 使其自动偏向场景最优策略。
...
#### 4.3.1 SchedulingMode 基线偏置

根据 batch_size 推导 SchedulingMode，然后生成基线偏置（与模型图无关）：

```
batch_size ≤ 1 (Latency) 基线:
  ...
  batch_flexibility       = 0.0   // batch=1 自然约束
...
```rust
impl StrategyArbiter {
    /// 主入口。每次调度决策时调用，batch_size 变化时 StrategyBias 自然变化。
    pub fn arbitrate(
        batch_size: usize,
        archetype: &GraphArchetype,
        profile: &DeviceProfile,
    ) -> StrategyBias {
        let mode = SchedulingMode::from_batch_size(batch_size);
...
impl HwOptEngine {
    pub fn solve(
        ...
        // Level 5: 循环依赖校正 (仅 batch_size > 1 时)
...
**变更**: 输入增加 `effective_m: usize`（从 `SchedulingMode::effective_m()` 获取）。

```rust
struct RooflineInput {
    profile: &DeviceProfile,
    model: &ModelConfig,
    effective_m: usize,     // 新增: 来自 SchedulingMode::from_batch_size(batch_size).effective_m()
}
```
...
// batch_size=1: effective_m = 1 → AI ≈ 2 → MemoryBound
// batch_size=64: effective_m = 64 → AI ≈ 128 → ComputeBound
```
...
```
batch_size ≤ 1 (Latency, bias: kv=0.5, weight=1.5):
...
batch_size > 1 (Throughput, bias: kv=1.5, weight=0.8):
...
for (producer, consumer) in fusion_candidates:
    raw_savings = fusion_savings(batch_size, ops)  // 原公式: 中间 tensor 写回字节数
    adjusted_savings = raw_savings / bias.fusion_cost_scale
    // fusion_cost_scale < 1.0 → savings 被 1/0.5 = 2× 放大

    raw_cost = fusion_overhead(batch_size, ops)    // 寄存器占用 + L1 压力
...
// batch_size ≤ 1 (Latency): parallelism_cost_scale = 1.5
//   → sync_cost × 1.5 → 同步更贵 → 倾向 wave=1
// batch_size > 1 + MoE (Throughput): parallelism_cost_scale = 0.25
...
// batch_flexibility = 0.0 是唯一的硬约束切入点（batch_size ≤ 1 时）
...
// speculative_decoding_value = 1.5 (batch_size ≤ 1) → 收益放大 1.5× → 更倾向启用
// speculative_decoding_value = 0.3 (batch_size > 1) → 收益缩小 → 不太启用
...
pub struct HwOptPlan {
    ...
    // 新增: 溯源信息
    pub batch_size: usize,           // 运行时 batch_size（驱动 SchedulingMode）
    pub graph_archetype: GraphArchetype,
    pub strategy_bias: StrategyBias,
}
```

**REQ-ARB-003**: `batch_size`、`graph_archetype`、`strategy_bias` 嵌入 HwOptPlan 以便下游模块溯源决策依据。`batch_size` 是运行时值，每次调度决策时可能变化；`graph_archetype` 和 `strategy_bias` 为只读。
...
Client::builder()
  ...
  [每次调度决策时]
  → SchedulingMode::from_batch_size(current_batch_size)  // ⑥ 从 batch_size 推导模式
      → batch_size=64 → SchedulingMode::Throughput { batch_size: 64 }
  → StrategyArbiter::arbitrate(64, archetype, device)    // ⑦ 仲裁
      → StrategyBias {
  ...
  → HwOptPlan { batch_size: 64, ... }                    // ⑨ 最终计划
  → JIT Codegen 生成机器码                                // ⑩ 编译
```

## 7. 约束与铁律

### REQ-ARB-004: SchedulingMode 由运行时 batch_size 决定

`SchedulingMode` 不存在用户指定入口，由运行时 `batch_size` 自动推导：`batch_size ≤ 1` → Latency，`batch_size > 1` → Throughput。batch_size 变化时 StrategyBias 自然变化，无需重新加载模型。JIT 编译产物支持任意 M（`SymDim::Symbolic("batch_size")`），不受模式影响。
...
下游模块禁止根据 `SchedulingMode` 或 `batch_size` 做直接 if/else 分支决策。所有模式差异必须通过 `StrategyBias` 传导。

```rust
// ❌ 禁止
if hw_opt_plan.batch_size <= 1 {
    max_batch = 1;
}
...
StrategyBias 不影响 JIT 编译产物——JIT 代码支持任意 M（`SymDim::Symbolic("batch_size")`），运行时 batch_size 自然落入不同 Roofline 区域。因此 `ModelJitCache` 的缓存 key 不包含模式信息：

```
cache_key = device_fingerprint + model_id
```

同一模型的 JIT 缓存在任何 batch_size 下复用同一份编译产物。batch_size 是运行时值，不在编译时维度中。

## 8. 实现位置

| 组件 | 文件 | 职责 |
|------|------|------|
| `SchedulingMode` | `src/engine/arbiter.rs` (新增) | 从 batch_size 推导调度模式 |
...
|--------|------------------------|------|
| RooflineAnalyzer | `SchedulingMode::effective_m()` (从 batch_size 推导) | 算术强度分类翻转 |
...
## 12. batch_size > 1 时 effective_M 循环依赖解决
...
```
阶段 1: 使用估算 M 完成初始求解
  estimated_M = estimate_batch_size(profile, model)
...
```rust
fn estimate_batch_size(profile: &DeviceProfile, model: &ModelConfig) -> usize {
...
**REQ-ARB-011**: batch_size > 1 时使用两阶段求解打破循环依赖。batch_size ≤ 1 时 effective_M=1，无循环。校正最多执行 1 轮，禁止迭代收敛。
...
### 15.2 向量 1: Llama-70B + batch_size=1 + CPU_AVX2

```
GraphArchetype: { compute: 0.95, memory: 0.60, parallel: 0.20, fusion: 0.85, pipeline: 0.15 }

阶段 1 — mode_baseline(Latency, batch_size=1):
...
解读: 极度偏好融合 (0.47)，深 epilogue (2.33)，batch=1 自然约束。
符合预期: Llama-70B + batch_size=1 → memory-bound GEMV，融合每省一字节 = 省一 cycle。
```

### 15.3 向量 2: DeepSeek-V3 + batch_size=64 + GPU_A100

```
GraphArchetype: { compute: 0.50, memory: 0.90, parallel: 0.99, fusion: 0.60, pipeline: 0.35 }

阶段 1 — mode_baseline(Throughput, batch_size=64):
...
符合预期: MoE 256 专家 + batch_size=64 → wave 最大化，专家权重异步预取。
```

### 15.4 向量 3: Phi-4 (3B) + batch_size=1 + CPU_AVX512
...
符合预期: 小模型 + batch_size=1 → memory-bound，GEMM 小所以 pipeline 和 fusion 都极重要。
```

### 15.5 向量 4: Qwen3-7B + batch_size=32 + GPU_A100

```
GraphArchetype: { compute: 0.73, memory: 0.40, parallel: 0.15, fusion: 0.80, pipeline: 0.45 }

阶段 1 — mode_baseline(Throughput, batch_size=32):
  fusion_cost_scale = 1.0
  parallelism_cost_scale = 0.5
  batch_flexibility = 1.0
...
符合预期: Dense 7B + batch_size=32 + GPU → 平衡策略，GEMM 够大不需要极端偏向。
```
...
```rust
#[test]
fn test_golden_vector_llama70b_batch1_cpu_avx2() {
    ...
    let bias = StrategyArbiter::arbitrate(1, &archetype, &profile);
    // batch_size=1 → SchedulingMode::Latency
...
### 16.2 ❌ 直接读 batch_size 或 SchedulingMode 做分支
...
// batch_size ≤ 1: batch_flexibility = 0.0 → max_batch = 0 → 实际 max(1, 0) = 1
```
...
| `04-API-DESIGN.md §2` Client Builder | 无 `inference_mode()` 入口，batch_size 从调度器自然推导 |
| `05-OPTIMIZATIONS.md` | MoE/SpecDecoding 模块消费 StrategyBias |
| `06-RUNTIME.md §3` ContinuousBatcher | batch 策略受 `batch_flexibility` 影响 |
| `DOCS/scheduling/jit-cache-protocol.md` | JIT 缓存 key 不含模式（batch_size 是运行时值） |
...
Client::builder()
  ...
  │
  └→ ClientBuilder::build_state(model_id, kind)   ← src/client.rs
       │
       ...
       │
       ├── [5] [每次调度决策时]
       │   StrategyArbiter::arbitrate(batch_size, &archetype, &hw_view)  ← src/engine/arbiter.rs
       │   └→ SchedulingMode::from_batch_size(batch_size)
...
**验证命令** (grep 确认非测试调用):
```bash
# StrategyArbiter::arbitrate 在真实路径被调用（接收 batch_size 而非 InferenceMode）
grep -rn "StrategyArbiter::arbitrate\|arbitrate_cpu" src/ | grep -v test | grep -v "mod tests"
# 期望: src/client.rs 或 src/engine/ 中有调用
...
# 期望: src/client.rs 或 src/engine/ 中有实例化和传递

# InferenceMode 不存在于非测试代码
grep -rn "InferenceMode" src/ | grep -v test | grep -v "mod tests"
# 期望: 空结果（InferenceMode 已被消灭）
```
