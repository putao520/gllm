...
`ModelKind` (`Chat` / `Embedding` / `Classifier` / `Reranker`) 描述**加载时的主 head 意图**——决定模型 auto_graph 架构推导、权重加载范围、默认 forward pass 路径。但实际应用中,同一份 generator LLM (如 SmolLM2-135M-Instruct) 的 `lm_head` 投影可以同时服务:
...
| 改变 | auto_graph 推导、权重形状、JIT 编译产物 | 后处理逻辑 (读取哪些 logit、是否提前退出) |
...
- **Generation Hooks** (`SPEC/04-API-DESIGN.md §3.1`): Hooks 作用于采样后 token veto,HR 在 lm_head 融合组 logits 计算时介入,不重叠。
...
2. **零 JIT 重编译**: 所有 HR 调用复用加载期编译好的 `CompiledLayer`,不触发重编译 (CLAUDE.md §5 REQ-JIT-CACHE-001 兼容)。
...
**数学定义** (Mega-Kernel `output_mode_selector` 支持 mid-layer exit):
...
**当前状态**: Mega-Kernel 的 `output_mode_selector` 支持 `EncodeToLayer` 模式，通过 Mega-Kernel callback table 在 anchor 层截断输出。`encode_to_layer` 通过 `output_mode_selector` 参数驱动 Mega-Kernel 在指定层退出。
...
实现上 Executor 直接调用 `MegaKernelExecutor::execute_score_tokens()` (output_mode_selector=4),对每个 target token id 从 scratchpad logits 中提取对应 score。
...
Mega-Kernel `output_mode_selector` 的 `EncodeToLayer` 模式已实现。`encode_to_layer` 通过 Mega-Kernel callback 在 anchor 层截断，返回 `Vec<f32>`。
...
1. 编译时: `auto_graph::build_compiler_graph()` 在 `intent_anchor_layer` 对应的层循环位置插入 `EarlyExit` op
...
验收 (Mega-Kernel output_mode_selector 已实现):
  (a) 返回 Vec<f32> 长度 == hidden_size
  (b) 不同 anchor layer 返回不同 embedding
...
运行时验证:
  ...
  构造 + token 解析 + mega-kernel execute_score_tokens 路径
```
...
| 依赖 | SgDetect/SgInject 共享内存 + Callback Table | Mega-Kernel `output_mode_selector` 完整前向产物 |
...
| `MegaKernelExecutor::execute_score_tokens()` (output_mode=4) | `src/engine/mega_kernel.rs` |
