...
2. **客户端原生调用 (Client Direct Invocation)**
   客户端 (CLI / Server) 侧引擎调度流须直接通过 `Executor` → `MegaKernelExecutor` 单次 CALL 完成推理，不经过 Backend trait 的 forward 方法。不再存在具有后台接管与降级逻辑的 `FallbackEmbedder`/`FallbackGenerator` 包装器。
...
**Graph 构建变更** (`auto_graph`):

Mega-Kernel 编译时 `auto_graph` 构建完整跨层 CompilerGraph，层间残差通过 `emit_loop` 内部寄存器传递:
...
**文件**: `gllm/src/arch/auto_graph.rs`, `gllm-kernels/src/compiler/fusion.rs`
...
> **SSOT**: REQ-JIT-GRAPH-001 (SymDim Symbolic Shape) 和 REQ-JIT-GRAPH-003 (auto_graph → Mega-Kernel 端到端) 均已完成 🟢。
...
| **缓存粒度** | 单算子独立编译（QRope, CachedGQA, Norm2） | 全模型单体 Mega-Kernel（MegaKernelExecutor 一次性编译整网） |
