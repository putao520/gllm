...
```
 ...
 │                                                                          │
 │  4. mega-kernel CALL (callback_table_ptr → GuardrailProbeCallback)          │
 │     ↓                                                                    │
 │  5. Mega-Kernel CALL (JIT callback table 内):                            │
 │     Mega-Kernel 内部执行所有层:                                            │
 │       ... JIT 机器码执行 ...                                              │
...
- `score > threshold` 时:
  ...
  3. Mega-Kernel JIT 机器码检测到 veto 标志，立即退出执行，返回空输出
  4. JIT GuardrailProbeCallback (post_node) 检测 logits 异常 → ExitEarly 返回空结果
...
| Backend 消费 | Mega-Kernel CALL（callback table 通过 `hook_ctx_ptr` 传入） |
...
| `Backend::apply_guardrail_probe` (Backend trait 方法,Executor 通过 callback_table_ptr 绕过) | `src/compat/cpu_backend.rs`, `src/compat/gpu_backend_macro.rs` |
