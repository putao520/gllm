...
> **背景**: `gllm-kernels/src/compiler/graph_builders.rs` 中 `decoder_model()` / `decoder_model_hetero()` / `build_layer_body()` 共 ~2000 行手写图构建与 YAML 模板不同步，导致 Gemma-4 E2B 的 8 个系统性 BUG（缺失 3 个 norm + layer_scalar + RoPE cache theta 错误 + 权重布局不匹配）
>
> **架构决策 — 绕过 FusedGraph 中间层**：gllm 已大量直接使用 `gllm_kernels::compiler::{CompilerGraph, OpKind}`（`graph_builders.rs`、`jit_helpers.rs`、`small_graph.rs`），不需要 FusedGraph / FusedOp 作为中间表示。新增 `ArchTemplate::to_compiler_graph()` 方法，直接从 YAML 模板构建 CompilerGraph，消除 FusedGraph→CompilerGraph 转换环节。
>
> **数据流**: `YAML 模板` → `ArchTemplate` → `to_compiler_graph(ResolvedConfig)` → `CompilerGraph` → `compile_mega_kernel()` → JIT 机器码
>
> **FusedGraph 路径保留**：`to_onnx_graph()` 和 FusedGraphExecutor 仍用于非 mega-kernel 的逐层编译路径（`Engine::build_from_artifact_cache`），REQ-UGS 仅替换 mega-kernel 编译路径。
