...
| **REQ-LOADER-022** | SafeTensors 张量驱动配置推导 | 从 SafeTensors 权重张量形状推导模型配置，优先于 config.json | 1. 从 embedding 张量推导 `vocab_size` (取较大维度)<br>2. 从 Q 投影张量推导 `head_dim` (`q_out / num_heads`)<br>3. 从权重张量 dtype 推导 `dtype_size`<br>4. **优先级**: 张量形状 > gllm.config 元数据 > config.json<br>5. **禁止**使用硬编码默认值 (Ω1: 真实性原则)<br>6. **关联**: ARCH-TENSOR-DRIVEN, DATA-TENSOR-DRIVEN | 🟢 已实现 (2026-02-08) |
| **REQ-LOADER-023** | ONNX 张量驱动配置推导 | 从 ONNX 权重张量形状推导模型配置，优先于外部配置 | 1. 从 embedding initializer 推导 `vocab_size` (取较大维度)<br>2. 从 Q 投影 initializer 推导 `head_dim` (`q_out / num_heads`)<br>3. 从 initializer dtype 推导 `dtype_size`<br>4. **优先级**: 张量形状 > ONNX attributes > 外部 config.json<br>5. **禁止**使用硬编码默认值 (Ω1: 真实性原则)<br>6. **关联**: ARCH-TENSOR-DRIVEN, DATA-TENSOR-DRIVEN | 🟢 已实现 (2026-02-08) |
