...
### REQ-LOAD-001: 异步并行模型加载

**描述**: 支持多分片 SafeTensors 文件的异步并行下载和解析

**问题背景**:
- 当前模型分片顺序加载，大模型（如 70B）加载时间长
- 多分片文件（model-00001.safetensors ~ model-000XX.safetensors）串行处理

**验收标准**:
- 多分片 SafeTensors 文件异步并行下载
- 多分片文件异步并行解析（内存映射 + 并行 mmap）
- 支持进度回调（下载进度、解析进度）
- 内存峰值控制：不一次性加载全部分片到内存
- 按需加载：只加载当前推理需要的权重
- 加载速度提升 ≥ 2x（对比顺序加载）
...
**描述**: 支持量化权重的原生 GPU kernel 推理，避免 dequantize 开销

**问题背景**:
- 当前量化模型每次推理都先 dequantize 到 f32，性能损失大
- Q4_0 模型每次 matmul 前都要解压到 4 倍内存

**验收标准**:
- 支持 Q4_0/Q4_K/Q8_0 等 GGUF 量化格式的原生 GPU kernel
- 支持 AWQ INT4 量化格式的原生 GPU kernel
- 直接在量化权重上计算，避免 dequantize 开销
- WGPU 后端实现（跨平台）
- CUDA 后端实现（高性能）
- 精度损失在可接受范围（vs dequantize 方案误差 < 1%）
- 推理速度提升 ≥ 2x（对比 dequantize + f32 matmul）
- 内存占用降低（Q4: 75%，Q8: 50%）
