...
> **背景**: gllm 历史上有两条编译/执行路径：(A) FusedGraph 路径 (YAML→OnnxGraph→FusedGraph(FusedOp)→CompilerGraph(OpKind)→JIT) 和 (B) Mega-Kernel 路径 (YAML→ArchTemplate→CompilerGraph(OpKind)→JIT)。路径 A 的 FusedOp→OpKind 转换存在系统性字段丢失（RoPE.partial_ratio 硬编码 1.0、FlashAttention.scale 丢失、GQA.num_kv_heads 丢失、MoERouting.top_k 丢失），且路径 A 的逐层编译无法支持 generate loop 融合。
>
...
>
> **唯一数据流**: `YAML 模板` → `ArchTemplate` → `to_compiler_graph(ResolvedConfig)` → `CompilerGraph` → `compile_mega_kernel()` → 单次 CALL → 推理结果

| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-UGS-001** | YAML→CompilerGraph 直接构建器 | `ArchTemplate` 新增 `to_compiler_graph()` 方法，直接从 YAML 模板构建 CompilerGraph（跳过 OnnxGraph/FusedGraph 中间层） | 1. `ArchTemplate::to_compiler_graph(&self, config: &ResolvedConfig) -> Result<CompilerGraph, TemplateError>` 方法实现<br>2. `op_type` 字符串 → `OpKind` 枚举完整映射表（Gather→OpKind::Gather, MatMul→OpKind::Gemm, SimplifiedLayerNormalization→OpKind::RmsNorm, SiLU→OpKind::Silu, Mul→OpKind::Mul, Add→OpKind::Add, RotaryEmbedding→OpKind::RoPE, Attention→OpKind::MultiHeadAttention, QkNorm→OpKind::QkNorm, ValueNorm→OpKind::ValueNorm, PerLayerEmbed→OpKind::PerLayerEmbed, PleSlice→OpKind::ColumnSlice, DualRotaryEmbedding→展开为两个 OpKind::RoPE）<br>3. `NodeDef::attributes` → `OpKind` 字段注入（theta→RoPE.theta, eps→RmsNorm.eps, head_dim→QkNorm.head_dim 等）<br>4. `only_if` 条件守卫复用 `eval_only_if_with_loop()` 逻辑<br>5. `repeat` 块展开为 N 组独立 OpKind 节点（与 `to_onnx_graph()` 相同展开策略）<br>6. mega-kernel 独有节点（SgInject, SessionKvRestore, MmHiddenInject, GenerateLoop）由 `MegaKernelBusinessConfig` 驱动注入，不在 YAML 中声明<br>7. 所有已有 YAML 模板（Llama/Qwen3/Gemma4/Phi4/Mistral/DeepSeek）构建结果与 `decoder_model()` / `decoder_model_hetero()` 等价 | 🟢 已实现 [SmolLM2 E2E 2026-04-27] |
| **REQ-UGS-002** | 异构层模板折叠 | `to_compiler_graph()` 根据 `config.attention_pattern[]` + `config.num_kv_shared_layers` 自动生成异构层图，替代手写 `decoder_model_hetero()` | 1. 按 `(attention_pattern[i], ffn_size[i])` 二元组去重，推导 K 个唯一层模板（如 Gemma-4 E2B → 4 模板: sliding_small/full_small/sliding_large/full_large）<br>2. 权重布局自动推导：从模板中收集所有权重张量，按模板分组计算 `PerLayerWeightLayout`，替代手写 `compute_per_layer_bytes`<br>3. `HeteroLayerLoopConfig` 从 `config.attention_pattern[]` + `config.intermediate_size[]` 自动生成，包含 `num_segments` 和 `layers_per_segment`<br>4. `is_kv_shared_layer` 跳过 consumer 层的 k_proj/v_proj/v_norm/qk_norm_k/rope_k，复用 `only_if` 守卫逻辑<br>5. `${donor_i}` 占位符解析：consumer 层的 Attention K/V 输入指向 donor 层输出 | 🟢 功能等价已实现（逐层展开；K 模板折叠为性能优化待后续） |
| **REQ-UGS-003** | 删除 graph_builders.rs 手写图构建 | `MegaKernelExecutor::compile_from_template()` 消费 `to_compiler_graph()` 产出，`InferenceCompiler::compile_mega_kernel_from_graph()` 接受外部 CompilerGraph | 1. `MegaKernelExecutor::compile_from_template()` 调用 `template.to_compiler_graph(&config)` + `compile_mega_kernel_from_graph()`<br>2. `gllm-kernels` 新增 `compile_mega_kernel_from_graph()` 接受外部 CompilerGraph<br>3. `gllm-kernels/src/compiler/graph_builders.rs` 中 `decoder_model()` / `decoder_model_hetero()` / `build_layer_body()` 函数保留（旧路径仍可用）<br>4. 所有 E2E 测试通过（SmolLM2, Gemma-4 E2B）<br>5. 新增模型只需写 YAML 模板 + 标量算子注册，零 Rust 代码修改 | 🟢 已实现 [SmolLM2 E2E 2026-04-27] |
...
| **REQ-UGS-006** | MoE 模型 Mega-Kernel 路径 | MoE 模型（GPT-OSS-20B、DeepSeek-V3、GLM-5）走 Mega-Kernel 编译路径，删除 executor 中 `if is_moe { None }` 跳过逻辑 | 1. `executor.rs` 中删除 `if is_moe { None }` 分支，MoE 模型正常进入 mega-kernel 编译<br>2. MoE YAML 模板包含 MoEGate/MoERouter/MoEDispatchPacked OpKind 节点<br>3. `compile_mega_kernel_vm()` 正确处理 MoE OpKind（不再返回空 vec）<br>4. 专家权重布局：mega-kernel 权重打包支持 per-expert 权重 stride<br>5. MoE 模型 E2E 测试通过（GPT-OSS-20B 输出非退化） | 🟢 已实现 |
| **REQ-UGS-007** | 删除 to_onnx_graph() 中间表示 | 删除 `ArchTemplate::to_onnx_graph()` 方法及 OnnxGraph 中间表示，`to_compiler_graph()` 是唯一的图构建出口 | 1. `src/arch/template.rs` 中 `to_onnx_graph()` 方法删除<br>2. `src/graph/types.rs` 中 OnnxGraph 相关类型保留（仍被 loader 等使用），FusedGraph 相关类型随 REQ-UGS-005 删除<br>3. 所有调用 `to_onnx_graph()` 的代码改为 `to_compiler_graph()`<br>4. 编译通过 + E2E 测试通过 | 🟢 已实现 |
