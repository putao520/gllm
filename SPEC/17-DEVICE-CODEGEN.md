...
> **实现状态**: ✅ REQ-CG-001~014 全部完成 — AArch64 VecCast/GatherLoad/ScatterStore/TableLookup/AtomicAdd + GPU 7 组 VmInstr lowering (scalar/gather/vector/LLM/GPR/callback/atomic) 三 dialect (PTX/HIP/Metal); 🟡 REQ-CG-015 auto_select device-aware 待增强
...
VecCmp 在 x86_64 也未实现（标记为 ❌）。需要补齐：

| Backend | 实现方案 |
|---------|---------|
| x86_64 | `VCMPPS` / `VCMPPD` (AVX2/AVX-512) |
| AArch64 | `FCMEQ` / `FCMGT` (NEON) + predicate (SVE) |
| GPU | PTX `setp.eq/gt/lt.f32` + predicated exec |
