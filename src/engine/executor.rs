...
pub struct Executor<B: Backend<E> + 'static, E: Element = f32> {
    ...
    /// Mega-Kernel Callback Table (ABI arg 20): C-style function pointer array
    /// for JIT→external callbacks. 8 slots × 16B = 128B, #[repr(C)].
    /// NULL fn_ptr = not registered, JIT skips via GprSkipIfNull.
    callback_table: MegaKernelCallbackTable,

    /// Raw pointer to leaked SgCallbackCtx for slot 0 (SG_KNOWLEDGE_RETRIEVE).
    /// Used to properly drop the ctx when clearing the callback.
    /// NULL = no ctx allocated.
    sg_callback_ctx_ptr: *const u8,
}
...
impl<B: Backend<E> + 'static, E: Element> Executor<B, E> {
    pub fn from_loader(
        ...
        Ok(Self {
            ...
            sg_callback_ctx_ptr: std::ptr::null(),
        })
    }
    ...
    pub fn set_sg_callback_shim(
        ...
        if let Some(ref mx) = self.sg_shared_memory {
            ...
            // Track leaked ctx for cleanup.
            self.sg_callback_ctx_ptr = cb_ctx_ptr;
        }
    }
