...
impl MegaKernelExecutor {
    ...
    pub fn execute_forward(
        ...
        // TEMP DIAG: diagnose layer loop execution
        {
            let sp_f32 = unsafe { std::slice::from_raw_parts(scratchpad.as_ptr() as *const f32, scratchpad.len() / 4) };
            let hidden = 384;
            let s = seq_len;
            eprintln!("[DIAG] sp total={} seq_len={}", sp_f32.len(), s);

            let out_norm: f32 = output.iter().map(|x| x * x).sum::<f32>().sqrt();
            eprintln!("[DIAG] output norm={:.4} first5=[{:.4},{:.4},{:.4},{:.4},{:.4}]",
                out_norm, output[0], output[1], output[2], output[3], output[4]);

            // Check sp[0] — this is where MeanPool reads (activation_alias output = hidden_0)
            for row in 0..s.min(3) {
                let r = &sp_f32[row * hidden..(row + 1) * hidden];
                let rn: f32 = r.iter().map(|x| x * x).sum::<f32>().sqrt();
                eprintln!("[DIAG] sp[0] row{} norm={:.4} first5=[{:.4},{:.4},{:.4},{:.4},{:.4}]",
                    row, rn, r[0], r[1], r[2], r[3], r[4]);
            }

            // Check sp[786432] and sp[196608] — non-zero scratchpad regions
            for &base in &[196608usize, 394752, 589824, 786432] {
                if base + hidden > sp_f32.len() { continue; }
                let r = &sp_f32[base..base + hidden];
                let rn: f32 = r.iter().map(|x| x * x).sum::<f32>().sqrt();
                if rn > 0.01 {
                    eprintln!("[DIAG] sp[{}] row0 norm={:.4} first3=[{:.4},{:.4},{:.4}]",
                        base, rn, r[0], r[1], r[2]);
                }
            }

            // Cosine sim between sp[0] meanpool and sp[786432] meanpool
            let mut pool0 = vec![0.0f32; hidden];
            let mut pool786 = vec![0.0f32; hidden];
            for t in 0..s {
                for j in 0..hidden {
                    pool0[j] += sp_f32[t * hidden + j];
                    if 786432 + t * hidden + j < sp_f32.len() {
                        pool786[j] += sp_f32[786432 + t * hidden + j];
                    }
                }
            }
            for j in 0..hidden { pool0[j] /= s as f32; pool786[j] /= s as f32; }
            let p0n: f32 = pool0.iter().map(|x| x * x).sum::<f32>().sqrt();
            let p786n: f32 = pool786.iter().map(|x| x * x).sum::<f32>().sqrt();
            let dot: f32 = pool0.iter().zip(pool786.iter()).map(|(a, b)| a * b).sum();
            let cs = dot / (p0n * p786n).max(1e-10);
            eprintln!("[DIAG] sp[0] meanpool norm={:.4} sp[786432] meanpool norm={:.4} cos_sim={:.4}", p0n, p786n, cs);

            // output vs sp[0] raw (not meanpooled)
            let out_vs_sp0: f32 = output.iter().zip(&sp_f32[..hidden]).map(|(a, b)| a * b).sum();
            let sp0_norm: f32 = sp_f32[..hidden].iter().map(|x| x * x).sum::<f32>().sqrt();
            let cs_out_sp0 = out_vs_sp0 / (out_norm * sp0_norm).max(1e-10);
            eprintln!("[DIAG] output vs sp[0] row0 cos_sim={:.4} (ratio out/sp0 = {:.4})", cs_out_sp0, out_norm / sp0_norm.max(1e-10));

            // Check: what scale factor was used in MeanPool?
            // output[i] ≈ meanpool[i] * 6 * scale, so scale = output[i] / (meanpool[i] * 6)
            let mut meanpool_exact = vec![0.0f32; hidden];
            for t in 0..s {
                for j in 0..hidden {
                    meanpool_exact[j] += sp_f32[t * hidden + j];
                }
            }
            let meanpool_norm_exact: f32 = meanpool_exact.iter().map(|x| x * x).sum::<f32>().sqrt();
            // output should equal meanpool_exact * scale
            // so scale = dot(output, meanpool) / dot(meanpool, meanpool)
            let dot_out_mp: f32 = output.iter().zip(&meanpool_exact).map(|(a, b)| a * b).sum();
            let dot_mp_mp: f32 = meanpool_exact.iter().map(|x| x * x).sum();
            let scale = dot_out_mp / dot_mp_mp;
            eprintln!("[DIAG] meanpool_exact norm={:.4} scale_from_output={:.6} expected_1_over_N={:.6} N_from_scale={:.1}",
                meanpool_norm_exact, scale, 1.0 / s as f32, 1.0 / scale);
        }
