        ...
        impl<E: Element> Backend<E> for $backend_ty<E> {
            ...
            fn embedding_forward_gpu_pure(
                ...
                #[cfg( $($cfg_pred)+ )]
                {
                    let _ = weights;
                    let bf = unsafe { &*(self as *const $backend_ty<E> as *const $backend_ty<f32>) };
                    let output_elems = tokens.len() * config.geometry().hidden_size;
                    gpu_encoder_forward_impl(bf, tokens, config, output_elems)
                        .map_err(|e| BE::$upload_err_variant(e))
                }
                #[cfg(not( $($cfg_pred)+ ))]
                {
            ...
            fn rerank_forward_gpu_pure(
                ...
                #[cfg( $($cfg_pred)+ )]
                {
                    let _ = weights;
                    let bf = unsafe { &*(self as *const $backend_ty<E> as *const $backend_ty<f32>) };
                    gpu_encoder_forward_impl(bf, tokens, config, 1)
                        .map_err(|e| BE::$upload_err_variant(e))
                }
                #[cfg(not( $($cfg_pred)+ ))]
                {
            ...
            fn classify_forward_gpu_pure(
                ...
                #[cfg( $($cfg_pred)+ )]
                {
                    let _ = weights;
                    let bf = unsafe { &*(self as *const $backend_ty<E> as *const $backend_ty<f32>) };
                    gpu_encoder_forward_impl(bf, tokens, config, config.geometry().num_labels().unwrap_or(2))
                        .map_err(|e| BE::$upload_err_variant(e))
                }
                #[cfg(not( $($cfg_pred)+ ))]
                {
