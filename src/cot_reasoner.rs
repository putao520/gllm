//! CoT Reasoner SDK — 任意 LLM 原生 Chain-of-Thought 推理 (REQ-COT-001..006)
//!
...
// ============================================================================
// ReasoningResponse / ReasoningStopReason (SPEC §7.3/§7.4)
// ============================================================================
...
impl Client {
    ...
    pub fn reason(
        ...
        // 预检: 模型加载状态 (早失败避免进入循环)
        let _state = self.require_state()?;
        let tpl = template.unwrap_or_default();
    ...
    fn run_manual_loop(
        ...
        for step in 1..=step_count {
            ...
            *total_tokens = total_tokens.saturating_add(estimate_token_count(&chunk_trimmed));
    ...
    fn run_auto_loop(
        ...
        // 选 32 是给 1k+ token 预算留充足空间(32 * AUTO_STEP_CHUNK = 4096),
        // 同时防御模型吐 1-token chunk 的极端情况。
...
// ============================================================================
// ReasoningBuilder (SPEC §7.2) — 由 GenerationBuilder::reasoning 返回
// ============================================================================
...
impl<'a> ReasoningBuilder<'a> {
    ...
    pub fn execute(self) -> Result<ReasoningResponse, ClientError> {
        self.client.reason(&self.prompt, self.mode, self.template)
    }
}
