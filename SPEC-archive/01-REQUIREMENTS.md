...
| **REQ-SCHED-019** | Batch Composition Token Budget | 调度层将 decode/prefill tokens 按预算组成物理 batch | 1. decode_ratio_cap = 0.6 (decode 最多占 60%)<br>2. 按 BatchOrderPolicy 排序填充<br>3. 生成 BatchManifest (§10.6)<br>4. AdaptiveChunkPolicy 动态调整 chunk 大小 | 🟢 已实现
| **REQ-SCHED-020** | Compact 触发与开销权衡 | 定义 Compact→Execute→Scatter 的触发条件和成本收益模型 | 1. 测量点: GEMM op 前的 active lane ratio<br>2. 触发: waste > 25% AND active ≥ min_threshold AND op 是 compute-bound<br>3. 禁止在 Attention op 上做 compact (memory-bound，无收益)<br>4. 开销模型: compact_cost < saved_flops × flops_to_mem_ratio | 🟢 已实现
...
| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-JIT-CACHE-001** | 全核心态级预编译 | 所有算子必须在模型加载阶段完成全网计算图预编译 | 1. 禁用独立算子的 JIT 缓存支持<br>2. `GraphExecutor` 为唯一的全模级别入口<br>3. 推理热路径中无任何单层或单算子重编译 | 🟢 已实现
| **REQ-JIT-CACHE-002** | CPU/GPU 算子与流程统一 | CPU 后端与 GPU 后端的图执行行为必须 100% 对齐 | 1. 消除 CPU 特有算子节点分支<br>2. 共同使用 `OpKind` 泛型拓扑与符号化绑定绑定(`SymDim`)<br>3. 完全消除 `GraphType` 的概念 | 🟢 已实现
| **REQ-JIT-CACHE-003** | 磁盘级持久化重构 | 依据版本信息缓存整图的二进制编译形态 | 1. L3 保存全模二进制 Kernel (以 Hash 为签名)<br>2. 跳过应用级调度管线编译<br>3. `Fallback` 防护机制健壮 | 🟢 已实现
| **REQ-JIT-CACHE-004** | 热路径零编译 | 推理 Decode Step 层循环中严禁用例 | `compile_graph` 调用频次必须恒为 0 | 🟢 已实现
| **REQ-JIT-CACHE-005** | SymDim::Symbolic 动态绑定 | SeqLen 通过 ShapeBinding 运行时绑定 | 1. 结合 JIT 的 变量发射寄存器 (Param Registers) 不触发内核重编译<br>2. 直接于执行队列替换 `total_seq_len` | 🟢 已实现
| **REQ-JIT-CACHE-006** | 零层级执行器 | 彻底废除按 Layer、按 Attention/FFN 进行调度的入口 | `ModelJitCache`、`FusedAttentionLayer` 这类概念不得出现任何一行存留 | 🟢 已实现
...
| **推测解码加速比** | ≥ 2× vs 标准 decode (同模型同精度) | EESD 自推测 + 各向异性树 + SIMD/Warp compact |

---

## 11.1 执行路径集成契约 (REQ-INTEGRATE)

> **关联架构**: 02-ARCHITECTURE.md §19 (EXECUTION-INTEGRATION-CONTRACT)
> **关联测试**: 06-TESTING-STRATEGY.md §13 (INTEGRATION-VERIFICATION)
> **核心法则**: 优化模块"实现完整但未接入执行路径"等同于"未实现"。SPEC 不接受"逻辑正确但未接入"作为完成状态。

| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-INTEGRATE-001** | 执行钩子声明 | 每个优化模块必须在 SPEC §19.3 注册表中声明 Hook Point、Decision Type、Effect Target | 1. §19.3 集成路径注册表包含该模块的完整声明<br>2. 状态标记为 🟢 Integrated<br>3. ❌ Not Integrated 不被视为完成 | 🟢 已实现 |
| **REQ-INTEGRATE-002** | 禁止日志即丢弃反模式 | 优化模块的决策产出禁止仅通过日志输出消费，必须通过结构化路径改变执行行为 | 1. 决策变量在 `log::debug!` 之后仍被消费（传入 Callback/Event/State）<br>2. `grep -rn "log::debug.*decision" src/engine/ src/jit/ \| grep -v callback` 返回 0 匹配<br>3. 审计无 LOG-AND-DISCARD 模式 | 🟢 已实现 |
| **REQ-INTEGRATE-003** | 集成测试强制要求 | 每个优化模块必须有集成测试证明"有/无模块时执行行为不同" | 1. 06-TESTING-STRATEGY.md §13.3 中有该模块的 TEST-INTEGRATE 条目<br>2. 测试代码证明 executor 行为在启用/禁用优化时不同<br>3. 禁止仅验证日志或模块内部函数 | 🟢 已实现 |
| **REQ-INTEGRATE-004** | Callback 注册验证 | 优化模块的 Callback 必须在 `build_step_callbacks()` 或等效注册点中注册 | 1. `src/engine/executor.rs::build_step_callbacks()` 中有对应的 `callbacks.push(...)` 调用<br>2. 条件分支正确（仅在模块配置后注册）<br>3. 无 `if false { /* TODO: register */ }` 等占位 | 🟢 已实现 |
...
> **核心法则**: 模型自身的浅层变体充当 Draft Model, 配合硬件指令级 batch 合并和各向异性推测树, 在**零额外模型权重**的前提下提升 decode 吞吐 2-3×。

| ID | 需求标题 | 描述 | 验收标准 | 状态 |
|----|----------|------|----------|------|
| **REQ-SPEC-001** | 自推测解码管线 | Executor 支持 Dual-Mode: 标准解码 + 推测解码, 通过 `step_speculative()` 自动调度 | 1. `SpeculativeDecodingState` 在 `from_loader()` 时初始化<br>2. Draft phase 使用 L2_hot 变体 (≈L/3 层)<br>3. Verify phase 使用 full 变体 (全部 L 层)<br>4. 自适应回退: 连续 3 轮 `avg_acceptance_rate < 0.3` 时回退到标准解码 | 🟢 已实现
| **REQ-SPEC-002** | 零参数 Draft Adapter | Draft model 的 logits 投影复用 **lm_head.weight** — 零额外模型权重 | 1. Adapter 图 = RmsNorm + MatMul(lm_head.weight^T), JIT 编译为 `FusedGraphExecutor`<br>2. Phase A (默认): 直接复用 `lm_head.weight` (共享引用)<br>3. Phase B (可选): 添加 `residual_delta`, 用 full model logits 蒸馏前 100 步 | 🟢 已实现
| **REQ-SPEC-003** | 各向异性推测树 | PLD spine + n-gram branch 的非均匀拓扑 | Goose Proposition 1: 组合树 accepted tokens ≥ 任一单源方法 | 1. Spine 由 prompt 中匹配 n-gram 延续 (接受率 60-80%)<br>2. Branch 从频率表取 top-k 替代候选 (接受率 5-15%)<br>3. Tree attention mask 以 CSR 格式存储<br>4. 单次 batched forward 验证全部 tree tokens | 🟢 已实现
| **REQ-SPEC-004** | EqSpec Batch 正确性 | Batched speculative decoding 的三条不变量 | 1. I1: 同 batch 共享 SpecTree 拓扑<br>2. I2: 单次 batched forward 验证<br>3. I3: 每 sequence 独立原子 KV commit/rollback | 🟢 已实现
| **REQ-SPEC-005** | 硬件指令级 batch 合并 | §9.1 Compact→Execute→Scatter 在异构 batch 场景的具体实现 | 1. CPU: AVX-512 `vcompressps`/`vpcompressd` 物理挤压 SIMD lanes<br>2. GPU: Warp Vote + `__shfl_sync` compact active threads<br>3. AArch64 SVE: predicate register 自动 mask inactive lanes<br>4. 自适应阈值: SIMD lane 浪费 > 25% 时触发 compact, 否则跳过 | 🟢 已实现
| **REQ-SPEC-006** | ADEPT 阴影 KV 填充 | Early-Exit token 在跳过层产生 KV 缺口时, 用低秩投影近似填充 | 1. ShadowProjector: JIT 编译的 [hidden→rank→kv_dim] 投影图<br>2. rank≥64 时 perplexity 下降 < 0.1<br>3. 在线蒸馏: 前 100 步用 full model 真实 KV 做蒸馏信号 | 🟢 已实现
| **REQ-SPEC-007** | 布局代价模型 | JIT 融合引擎用 `LayoutCost` 量化不同数据布局的硬件代价 | 1. 四个维度: CacheLineWaste / TLBPressure / BankConflict / SIMDLaneWaste<br>2. 加权代价公式用于融合路径选择<br>3. 硬件参数由 `DeviceProfile` 驱动, 不硬编码 | 🟢 已实现
| **REQ-SPEC-008** | 稀疏 KV 布局策略 | Early-Exit 产生的 KV 空洞的三种物理表示 | 1. Dense+Shadow (推荐): 全量分配 + 影子投影填充, 零运行时分支<br>2. Per-Layer Bitmap: 每层 bitmap 标记有效 token, ~3% 额外访存<br>3. CSR Compressed: 层→token CSR 格式, 极端稀疏场景<br>4. 约束: 推测解码场景必须使用 Dense (verify phase 自然产生完整 KV) | 🟢 已实现
| **REQ-SPEC-009** | 编译时全量预编译 | Adapter + ShadowProjector 在模型加载时编译, 与变体编译同时完成 | 1. `build_adapter_graph()` 产生 RmsNorm+MatMul 图<br>2. `build_shadow_projector_graph()` 产生 Linear↓+Linear↑ 图<br>3. 两者与 PolymorphicExecutor 变体共享同一 `ArtifactCache`<br>4. 推理热路径零编译 (REQ-JIT-CACHE-004) | 🟢 已实现
| **REQ-SPEC-010** | 输出分布精确性 | 推测解码的输出分布与目标模型精确一致 (Speculative Decoding 数学保证) | 1. Draft logits 仅用于 candidate selection (不影响最终输出)<br>2. 验证步骤始终使用 full model logits<br>3. 接受/拒绝遵循 rejection sampling (exact target distribution)<br>4. Adapter 近似只影响接受率 (效率), 不影响输出质量 | 🟢 已实现
| **REQ-SPEC-011** | SAGUARO 多 GPU 并行推测解码 | ≥2 GPU 时自动启用 SAGUARO 模式: 独立 Draft GPU + Target GPU Pool, draft/verify 流水线并行 | 1. 单 GPU → EESD (浅层变体做 draft)<br>2. ≥2 GPU → SAGUARO (独立 draft GPU, draft+verify 并行)<br>3. NCCL 通信每轮仅一次 `all-gather` 同步 (~10-100μs/round)<br>4. Draft GPU 使用 L2_hot 变体, Target GPU 使用 full 变体 + TP | 🟢 已实现
| **REQ-SPEC-012** | 几何扇出分配 (Theorem 12) | 同时验证 F×(K+1) 个分支, 位置 k 的分支数 `F_k = F_0 × a_p^(k/(1+r))` | 1. a_p 从历史 `avg_acceptance_rate` 滑动平均<br>2. r = draft_latency / target_latency (运行时统计)<br>3. 总分支数受 NCCL 通信预算约束: F×(K+1) ≤ 64<br>4. 更多计算分配给"大概率短接受"位置 (非均匀) | 🟢 已实现
| **REQ-SPEC-013** | 缓存感知采样 (Theorem 15) | 对已缓存 top-F token 概率乘以 C < 1, 集中分布质量到缓存 token, 提升缓存命中率 | 1. SpeculationCache 存储 draft logits (跨轮复用)<br>2. 选择使 P(cache_hit) × P(accept\|hit) 最大化的 C<br>3. 每轮 speculation round 后全量刷新 cache<br>4. 缓存命中时跳过 draft GPU 计算 (直接 lookup) | 🟢 已实现
| **REQ-SPEC-014** | 自适应回退策略 (Theorem 17) | Cache 未命中时根据 batch 大小切换备份策略 | 1. 低 batch (b < b*) → 慢速 (draft model 实时生成, 质量高)<br>2. 高 batch (b ≥ b*) → 快速 (n-gram 随机生成, 延迟低)<br>3. 阈值 b* 由解析公式推导 (平衡延迟与接受率)<br>4. 与 EESD 模式共享 n-gram 索引基础设施 | 🟢 已实现
| **REQ-SPEC-015** | 稀疏 KV 策略选择算法 | 基于 SparseKvContext 动态选择最优稀疏 KV 布局策略 | 1. 推测解码活跃 → 必须 DenseShadow (verify phase 自然产生完整 KV)<br>2. sparsity > 80% → CsrCompressed (极端稀疏, 内存最优)<br>3. sparsity > 20% → PerLayerBitmap (中等稀疏, mask 开销可控)<br>4. else → DenseShadow (推荐, 零运行时分支)<br>5. 支持每次 batch 前重新评估 (mid-sequence 切换) | 🟢 已实现
| **REQ-SPEC-016** | ShadowProjector 运行时生命周期 | 定义 ADEPT 阴影 KV 填充的完整运行时流程 (编译→填充→校准) | 1. 编译: 模型加载时与变体编译同时完成, 存入 ModelJitCache<br>2. 填充: 每次 batch 后遍历 early-exit tokens, forward projector 写入 KV 缺口<br>3. 校准: 前 100 步 L2 蒸馏 (full KV vs shadow KV), SGD 微调 W_down/W_up<br>4. 收敛: perplexity_delta < 0.1 | 🟢 已实现
| **REQ-SPEC-017** | Sparse KV + PagedAttention 交互规则 | 定义稀疏 KV 策略与 PagedAttention 页面管理的交互约束 | 1. DenseShadow/PerLayerBitmap: 全量 page 预分配, 零 page 管理变化<br>2. CsrCompressed: 仅分配有效 entries 的 page, 需 sparse page allocator<br>3. 前缀共享: exit 层序列完全一致的 sequences 才能共享前缀 KV pages<br>4. Rollback: rejected tokens 的 KV pages 整页回收, bitmap clear bits | 🟢 已实现
