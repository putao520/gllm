...
**完成状态**: `quantized_linear()` 已在 forward 路径中调用，检测量化权重时直接走 `Backend::quantized_matmul()` 而非先隐式解包浮点。
...
**完成状态**: 7 个 IQ polar decode 函数已完整实现（codebook 查表），8 个单元测试已通过。`iq_stub_decode` 死代码已移除。
