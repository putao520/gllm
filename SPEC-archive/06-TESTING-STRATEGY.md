...
## 13. 执行路径集成验证规则 (INTEGRATION-VERIFICATION)

> **关联架构**: 02-ARCHITECTURE.md §19 执行路径集成契约
> **关联需求**: REQ-INTEGRATE-001 ~ REQ-INTEGRATE-003
> **核心法则**: 优化模块的单元测试只证明"模块内部逻辑正确"，不证明"模块被接入执行路径"。集成验证证明"模块改变了执行行为"。

### 13.1 集成测试铁律 (INTEGRATION-TEST-IRON-LAW)

| 铁律 | 说明 |
|------|------|
| 每个优化模块必须有集成测试 | 单元测试 ≠ 集成测试 |
| 集成测试必须证明执行行为改变 | 证明"有模块"和"无模块"时，Executor 输出或行为不同 |
| 集成测试禁止仅验证日志输出 | `assert!(log.contains("skip"))` ❌ — 必须验证计算结果或执行路径 |

### 13.2 集成测试模板

```rust
/// TEST-INTEGRATE-{模块名}: {模块} 执行路径集成验证
///
/// **关联需求**: REQ-INTEGRATE-002
/// **关联架构**: 02-ARCHITECTURE.md §19.5 IC-3
/// **测试类型**: 集成测试
///
/// **测试步骤**:
/// 1. 构造 Executor（无优化模块）→ 执行 forward → 记录 baseline 行为
/// 2. 构造 Executor（启用优化模块）→ 执行相同 forward → 记录优化行为
/// 3. 对比两者行为差异
///
/// **期望结果**: 两者行为**必须不同**（如：forward 调用次数减少、
/// 输出 token 序列不同、执行耗时不同等）
#[test]
fn test_{module}_integration() {
    // Baseline: 无优化
...
| TEST-ID | 模块 | 验证方法 | 状态 |
|---------|------|---------|------|
| TEST-INTEGRATE-CALLBACK | CallbackChain | 空链 vs 有回调时 forward 行为不同 | 🟢 `src/graph/layer_callback.rs` tests |
| TEST-INTEGRATE-SPEC | Speculative Decoding | draft_budget 非零时 build_batch 输出不同 | 🟢 `src/scheduler/batcher.rs` tests |
| TEST-INTEGRATE-EARLY-EXIT | Early Exit | ExitEarly 回调跳过后续层 | 📋 待补充 |
| TEST-INTEGRATE-GATE-SKIP | Gate Skip | SkipThisNode 回调减少 FFN 计算 | 📋 待补充 |
| TEST-INTEGRATE-RAG | Late-Fusion RAG | InjectHidden 回调修改 hidden state | 📋 待补充 |
| TEST-INTEGRATE-MOE | MoE Dispatch | pre_node 回调设置路由上下文 | 📋 待补充 |
...
| 仅测试模块内部函数 | 内部正确 ≠ 接入正确 | 测试完整 Executor 流程 |
| `assert_eq!(callback.pre_node().priority(), 60)` | 只验证元数据，不验证效果 | 验证回调返回 SkipThisNode 后 Executor 真的跳过了节点 |
| Mock Executor | 绕过了真实集成路径 | 使用真实 Executor + 真实 Graph |
