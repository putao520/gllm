...
- **Q5_K_M 多层 E2E 乱码 (独立 bug, 非 decode)**: 28层 E2E 仍 output='遇浚lar菊花...' (非 Paris). Q5_K decode 已根治 (1层 cos 0.9998, JIT vs scalar 逐元素 diff=0). **第一性原理二分 (Q6_K 模型决定性测试)**: Q6_K 模型 (全层 Q6KDecodeStep) N=2 cos 0.9999 完美 → 排除 Q6KDecodeStep reentrancy. **test_q5k_q6k_mixed_program SIGSEGV 是 harness 问题** (Q5K+Q5K 同 program 也 SIGSEGV → 非 Q5K+Q6K 混合格式冲突, 是裸 StackArg 布局与 native call stack 冲突; 真实 mega-kernel 用不同 ABI 不触发). **architect save-order 根因证伪**: regalloc dump 显示 rax 是 ISA Lower 独占 scratch (不进 RegAllocator 可分配池), counter/byte_offset 分配到 rbx/r12-r15/rcx/rdx/rsi/rdi 不在 rax/r10, block_base/lane_offset Spilled 但 resolve_gpr_read 用 rax(scratch) 不影响活跃 VReg → save-order 修复对 Q5_K_M N=2 零影响 (cos 不变 0.7839). 已排除: decode 逻辑/pack/layer-loop weight base/Q6K reentrancy/BF16 harness/VReg 冲突(harness 假象)/save-order(architect 根因证伪). **architect agent 决定性诊断 (GLLM_DEBUG_BUFFER_ALLOC=1 编译时 dump 对比)**: Q5_K_M vs Q6_K 编译时 buffer 分配**完全一致** (slot offset/size/lifetime/total_bytes 全同, 2348810240), 唯一差异仅 QuantGemmSpec.quant_type 字段 (Q5K vs Q6K); activation 映射两者都是 `TensorId(30)→idx=0`; ping/pong sentinel offset/size 全同; 无 intermediate 与 ping/pong overlap. → **证伪 architect ping/pong overlap 方向**: bug 不在编译时 buffer 布局, 在运行时 Q5KDecodeStep 与 Q6KDecodeStep 混合执行的交互 (buffer 布局对, 但运行时 decode step 读写有 Q5K 特有问题). Q5_K_M 混合布局: Q5K(q/k/o/gate/up_proj) + Q6K(v/down_proj/lm_head); Q6_K 模型全 Q6K. 两者走同一 HighBitMerge kernel, 同一 a_dtype=ctx.accum_dtype(F32, 激活 F32 非 G2b BF16 越界场景). **weight_stride 数值验证 (GLLM_DEBUG_WEIGHT_STRIDE=1)**: Q5_K_M weight_stride=11379712 (理论值 4096+1441792+720896+860160+512+512+1441792+4096+2162688+2162688+2580480=11379712 完全匹配); Q6_K weight_stride=12911616 (理论 12911616 完全匹配). 两模型 stride 计算正确, layer1 weight base = global_weight_bytes + 正确 stride, 寻址公式+数值全对. **所有静态分析穷尽且全部排除** (decode/pack/layer-base/weight 传播/Q6K reentrancy/BF16 harness/VReg冲突/save-order/activation_bytes 公式/编译时 buffer 布局/weight_stride 数值). **第 12 方向 (Q5KDecodeStep vs Q6KDecodeStep native call lowering 对称性, architect consult agentId a07ca9b44c07055a3)**: 8 个对称点验证 — save_gprs 列表完全一致 (14 元素 [rax,rbx,rcx,rdx,rsi,rdi,r8-r15], 4 个 decode step 全同, lower_instr_dispatch.inc.rs:3948/4039/4124/4207); push/restore LIFO 对称 (callframe.inc.rs SymbolicSaveFrame); YMM 保存对称 (15 个 = 16-dst); 栈对齐一致 (600%16=8 → call 后 16B 对齐); dynamic stack alignment 对称; native helper 纯标量 extern "C" (q5k_decode_step_native quant_gemv.rs:2053, q6k_decode_step_native :1987); emit 点对称 (auto_select.rs:1536/1569). → **Q5K/Q6K native call lowering 完全对称, 不存在 Q5K 漏保存的寄存器, 第 12 方向排除**. **architect 新洞察**: 静态 lowering 对称 ≠ 运行时 VReg live-range 跨回边正确. 建议方向: 运行时 block_base VReg 在层循环第 2 次迭代的实际物理值是否正确 — 静态 lowering 保证 save/restore 对称, 但**不保证 RegAllocator 在层循环回边时正确处理 live-range** (block_base/lane_offset Spilled 状态跨回边的 reload 时机). **第 13 方向 (VmProgram 层循环回边序列对比, GLLM_DUMP_MEGA=1 dump Q5_K_M vs Q6_K N=2 VmProgram)**: Q5_K_M 16350 VmInstr vs Q6_K 16794 VmInstr. 结构 diff (归一化 quant_type/stride/offset/value 后) 显示**完全对称** — 差异全是量化格式的正确参数: (1) line 73 `GprLoadImm value:1`(Q5K high_bits=1) vs `value:2`(Q6K high_bits=2) — 正确; (2) AddPtr offset 16/0(Q5K qh@+16,d@+0) vs 128/208(Q6K qh@+128,d@+208) — 正确布局; (3) stride 704(Q5K 4×176) vs 840(Q6K 4×210) — 正确; (4) DecodeStep 分布 Q5_K_M=18 Q5KDecodeStep+9 Q6KDecodeStep(27) vs Q6_K=33 Q6KDecodeStep — 混合 vs 纯 Q6K 合理差异. **层循环 weight_ptr 传播验证**: VmProgram line 107 `LoadPtr VRegId(50), AbiArg(1)` + line 109 `VRegId(51)=VRegId(50)+VRegId(49)` (byte_offset) + line 111 `VRegId(52)=VRegId(51)+layer_blob_base_offset` — weight_ptr 每次层迭代从 ABI 重载, 不依赖跨回边 VReg 持久 → **排除 architect "VReg live-range 跨回边" 假设** (VmProgram 层 weight_ptr 重载正确). **VmProgram 层无异常, bug 在 VmProgram→机器码 lowering 后 (RegAllocator 物理寄存器分配 + ISA lowering)**: Q5_K_M 27 个混合 DecodeStep (18 Q5K+9 Q6K) vs Q6_K 33 个纯 Q6KDecodeStep, RegAllocator 可能在混合 DecodeStep 的物理寄存器布局上产生 Q6_K 不会触发的冲突. **第 14 方向 (RegAllocator 物理寄存器分配层, architect consult agentId adecf1a90d91cbbd1 + GLLM_REGALLOC_DEBUG=1 dump 确凿验证)**: architect 7 点静态验证 (save_gprs 14 GPR 完整覆盖 RegAlloc 11 可分配 GPR; push/restore LIFO 对称; resolve_gpr_read 总从 spill slot 重载不假设 scratch 持久; VmProgram 对称→regalloc 输入对称→算法确定性→分配对称; NativeCall 0 次 Q5K/Q6K 走内联 save/restore). **dump 事实确证 (Q5_K_M vs Q6_K regalloc 对比)**: (1) 物理 GPR 使用完全相同 — 两者都用 PhysGpr(1,2,3,6,7,8,9,12,13,14,15) = rcx/rdx/rbx/rsi/rdi/r8/r9/r12/r13/r14/r15, 频次比例近同 (Q6_K 略多因 33 vs 27 DecodeStep); rax(0)/r10(10)/r11(11) scratch 两者 0 次分配 (不在 RegAlloc 池); rsp(4)/rbp(5) 两者 0 次. (2) spill 布局前 30 VReg 逐字节相同 (v0→off0, v1→off8, ... v62→off232 完全一致). (3) spill offset 0 重复 — 无 slot 冲突 (每 VReg 独立 offset). (4) Q5_K_M 4213 spills vs Q6_K 4309 spills (差 96 = 27 vs 33 DecodeStep 合理差异). → **RegAlloc 物理寄存器分配层确凿排除** (非仅 architect 推理, dump 事实). **architect knowledgeGap (DEC-KB-DRIVEN-001 顶层范式第5条)**: RegAllocator 在混合 native-call 序列下的运行时行为无训练数据覆盖, 建议建库 docs/domain-knowledge/regalloc-native-call-interaction.md (SysV AMD64 ABI §3.2 + LLVM 线性扫描 regalloc). 但 knowledgeGap 不阻断第14方向排除 — RegAlloc 层结构性对称已通过代码审查+dump 验证确认, 不确定的只是运行时机器码执行后物理寄存器实际值 (需 DAP). Q5_K_M N=2 崩的真正根因仍未定位 — 需用可信 mega-kernel 级 DAP runtime 调试 (非静态分析, 非裸 StackArg harness, 非编译时 buffer 布局, 非数值 stride 错, 非 native call lowering 不对称, 非 VmProgram 层循环序列异常, 非 RegAlloc 物理寄存器分配). **第 15 方向 (★LoopInvariant 覆盖警告强信号, debug-ops skill + verify 输出)**: Q5_K_M mega-kernel verify 报 **4651 个 LoopInvariant VReg 在循环内被覆盖** 警告 ("LoopInvariant VRegId(N) overwritten at instr[M] inside loop[L0,L1]. Consider retagging as LoopCarried"). 层循环 [105,5152] 内 Q5_K_M 有 403 个覆盖 (341 unique VReg), Q6_K [105,5300] 有 421 个覆盖 (355 unique VReg). 两者都有大量警告但 Q6_K N=2 完美 → 警告本身可能良性. **Q5_K_M 独有 VReg**: VRegId(105), VRegId(1071-1087 等在层循环内被覆盖且 Q6_K 无对应 (Q6_K 独有 1030-1039). VReg 编号差异 (量化格式不同产生不同 VReg 序列), 非结构性. **警告语义**: RegAlloc 把 VReg 标记为 LoopInvariant (期望跨循环迭代保持值, 分配到 callee-saved 或不 spill), 但 lowering 代码在循环内覆盖了它 → 回边后值已变 → 若该 VReg 在第 2 次迭代被使用 → 用错误值. 完美匹配 N=1 对 (只1次迭代覆盖不影响) / N=2 崩 (第2次用被覆盖值) / N=4 全零 (累积). **未确证**: 哪个被覆盖的 LoopInvariant VReg 在 Q5_K_M 第 2 次迭代被使用且 Q6_K 不触发 — 需 architect 分析 VReg live-range 跨回边 + LoopInvariant 标记逻辑 (reg_alloc.rs derive_loop_invariants). **第 15 方向排除 (architect consult agentId a469b07233123f612)**: LoopInvariant 覆盖警告**良性**. verify.rs:362-364 注释明确 GEMM BLIS pattern 的累加器/broadcast 临时变量循环内多次写是设计意图, LifecycleTag 只是分配优先级提示不影响正确性. **weight_row_ptr 语义** (reg_alloc.rs:876-877,949): 循环外定义 + 循环内先读(计算 blk_ptr)后写(+=stride) = LoopCarried 语义, infer_lifecycle 规则2 优先规则3 标为 LoopInvariant 但 Pass 3 正确延展 last_use 到 LoopEnd. **物理分配基于 live interval (def_point/last_use) 非 LifecycleTag**: LoopInvariant 和 LoopCarried 优先级相同(都是2), Pass 3 正确延展 last_use → 物理寄存器分配正确. Q6_K 421 个同类警告但 N=2 完美 → 警告不导致运行时值错误. **architect knowledgeGap**: regalloc-native-call-interaction.md 不覆盖 LifecycleTag 标记逻辑, 建议补库 regalloc-lifecycle-tag-semantics.md (LoopInvariant vs LoopCarried 在"先读后写"VReg 的已知偏差). 但 knowledgeGap 不阻断排除 (LifecycleTag 只是提示, 物理分配基于 live interval 正确). **累计 15 个静态方向全排除, bug 明确需可信 mega-kernel 级 DAP runtime 调试** — 层循环回边处设断点检查第2次迭代开始时关键 VReg(权重指针/activation buffer 地址/KV cache 指针)的物理寄存器实际值. debug_process MCP check_env 超时(MCP server 卡死) + 系统仅 gdb 无 lldb + JIT 代码无调试符号 → DAP 调试 JIT 机器码面临实际困难, 需评估替代方案(Rust 端插桩 / gdb attach JIT 地址). **第 16 方向 (★JIT 最终机器码静态验证, 用户第一性原理: 算法对+JIT代码对+布局对=结果对)**: 用户指出 JIT 是规范性死东西, 先确认算法(已验证 diff=0)再确认 JIT 出来的代码, 寄存器/堆栈/内存布局也要验证. GLLM_DUMP_JIT_CODE=1 dump Q5_K_M(1245184B) vs Q6_K(1253376B) 最终机器码, objdump 反汇编对比: (1) call rax 数 Q5_K_M=27(18 Q5K@0x6418397d6390 + 9 Q6K@0x6418397d6e60) vs Q6_K=33(全@0x62b330d0ae60) — 完美匹配 VmProgram 统计; (2) save_gprs push 序列对称 — 两者都 push rax,rbx,rcx,rdx,rsi,rdi,r8-r15,pushf(14 GPR+pushf); (3) restore pop 序列对称 — LIFO 逆序 popf→r15→...→r8→rdi/rsi/rdx/rcx/rbx/rax; (4) YMM 保存/恢复对称 — 16 vmovups; (5) block_base/lane_offset 参数传递对称 — `mov rax,[rbp-0x110]` + `mov r10,[rbp-0xe0]` + `mov rdi,rax` + `mov rsi,r10` (两者 spill offset 完全相同 0x110/0xe0); (6) 层循环回边对称 — 3 处 `add r15/r12/r14, weight_stride` (Q5_K_M 0xada400 vs Q6_K 0xc50400, 寄存器相同); (7) activation swap 对称 — 9 个 `xchg r10,r11`. → **JIT 最终机器码完全对称** (save/restore/call/回边/swap 全对称, 差异仅 stride 数值 + call 目标地址). 用户第一性原理: 算法对+JIT代码对+布局对 → 结果应对, 但 Q5_K_M N=2 崩 → **矛盾**: 所有"规范性的死东西"都对, 结果却错. → **bug 不在代码本身, 在运行时数据**: 同样机器码喂 Q5K 权重 vs Q6K 权重, Q5K 路径出错. 剩余嫌疑: Q5K 权重 pack 后的实际字节内容 (sig 匹配但未逐 block 验证) / native helper 读 block 的运行时实际地址. 需验证 Q5K decode 输入 block 字节 vs scalar reference 输入 block 字节是否一致. **第 17-19 方向 (★用户第一性原理: 算法+JIT代码+布局+数据全验证)**: 用户指出 JIT 是规范性死东西, 静态验证无需 GDB. 第17: blob L0.q_proj block0 == GGUF 176/176 逐字节匹配 (pack 正确). 第18: blob L1.q_proj block0 == GGUF blk.1 176/176 (layer1 weight base 正确, stride+rel_offset 对). 第19: 运行时 Q5KDecodeStep #0 block_base 指向验证 — GLLM_DEBUG_Q5K_BLOCK 插桩 dump 运行时 #0 的 d=0.000063 head16=[1b,04,18,13,...]. **误判修正**: 初看 d=0.000063 ≠ token_embd block0 d=0.000080 以为运行时读错地址, 实际 #0 是 embed gather 读 token[0] 对应 row block (非 token_embd 整体 block0). token row stride=704 bytes (hidden=1024/block_size=256 × block_bytes=176), token_id[0]≠0 → 读 token_embd 中间 row 的 block. GGUF 暴力搜索 head16=[1b,04,18,13] 3 处匹配 (offset 134134880/345153040/393795536), 是某 token row 的 block0 — 运行时数据**正确**, 之前"d 不匹配"是对比错 block 的误判. **累计 19 方向全排除**: 所有"规范性死东西" (算法/JIT机器码/寄存器/堆栈/内存布局/权重数据 layer0+layer1/运行时 block_base) 全部验证正确, 但 Q5_K_M N=2 仍崩. 矛盾: 一切都对, 结果却错. **第 20 方向 (★Q5K/Q6K GEMM 代码路径澄清 + QhBitExpand dead code)**: VmProgram 显示 Q5K GEMM 用 Q5KDecodeStep (monolithic, 18 个), 非 QhBitExpand. 但 emit_gemm_highbit_from_plan (quant_gemm.inc.rs:487) 用 BlockUnpackMode::QhBitExpand (line 624). **矛盾澄清**: Q5K/Q6K GEMM 实际走 Gemv/General 模式的 `auto_lower_trace_raw` (quant_gemm.inc.rs:913) → DecodeTraceBuilder → Q5KDecodeStep, **不走 emit_gemm_highbit_from_plan 的 QhBitExpand**. emit_gemm_highbit_from_plan + QhBitExpand 是 **dead code** (被 monolithic DecodeStep 替代但未删). QhBitExpand 的 `qh_bit_value = 1<<(high_bits*4)` (Q5K=16 对, Q6K=256 超 byte 是 bug, 但 dead code 不影响运行). 第 20 方向: 代码路径澄清, 非 bug 根因 (QhBitExpand dead code, Q5K GEMM 走 monolithic DecodeStep 已验证对称). **★第 21 方向 (agent B ac649099d7c2c27ed activation 运行时内容, 关键发现)**: agent B 写 tests/test_diag_activation_runtime.rs 对比 Q5_K_M vs Q6_K N=2 的 activation pong buffer (offset 167772160) 内容. **KEY FINDING: Q5_K_M N=2 pong buffer 全零**, Q6_K N=2 pong 完美保留 layer0 输出 (cos=1.0). → layer1 执行破坏了本应保留 layer0 输出的 pong buffer (运行时数据损坏的直接证据). 这与 N=1 cos 0.9998 (layer0 写 pong 正确) / N=2 cos 0.78 (layer1 读到全零 pong) / N=4 全零 (累积) 症状完美匹配. 静态全对称但运行时 pong 被清零 → bug 在 layer1 写入地址或 swap 时机 (运行时, 非静态). agent B 分析中, 待最终结论.
- classic.rs INTERLEAVED vs SPLIT (独立 backlog).
